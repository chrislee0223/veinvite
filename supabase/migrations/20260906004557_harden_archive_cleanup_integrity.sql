...
begin
  ...
  loop
    if p_dataset_key='app_product_events' then
      select count(*),max(e.received_at) into v_current_count,v_latest_source_change
      ...
      select count(*),max(s.updated_at) into v_current_count,v_latest_source_change
...
as $$
declare
  v_policy_days integer; v_retention_days integer; v_cutoff date; v_day date;
  v_days integer:=0; v_events bigint:=0; v_deleted bigint:=0; v_unarchived date;
begin
  select p.hot_retention_days into v_policy_days
  from public.veinvite_retention_policy_versions p
  where p.dataset_key='app_product_events' and p.effective_from<=clock_timestamp()
  ...
  select min(q.usage_date) into v_unarchived
  from (select distinct e.usage_date from public.app_product_events e where e.usage_date<=v_cutoff) q
  where not public.is_analytics_date_verified_archived('app_product_events',q.usage_date);
  if v_unarchived is not null then raise exception 'raw product analytics cleanup blocked: % does not have a current VERIFIED archive manifest',v_unarchived; end if;
  for v_day in select distinct e.usage_date from public.app_product_events e where e.usage_date<=v_cutoff order by 1 loop
    if not public.is_analytics_date_verified_archived('app_product_events',v_day) then raise exception 'raw product analytics cleanup blocked at delete time: % archive is stale or unverified',v_day; end if;
    ...
    v_events:=v_events+v_deleted; v_days:=v_days+1;
  end loop;
...
declare
  v_policy_days integer; v_retention_days integer; v_cutoff date; v_day date;
  v_days integer:=0; v_sessions bigint:=0; v_visitors bigint:=0; v_deleted bigint:=0; v_unarchived date;
begin
  select p.hot_retention_days into v_policy_days
  from public.veinvite_retention_policy_versions p
  where p.dataset_key='app_usage_sessions' and p.effective_from<=clock_timestamp()
  order by p.effective_from desc,p.created_at desc limit 1;
  ...
  v_cutoff:=(clock_timestamp() at time zone 'Asia/Seoul')::date-v_retention_days;
  select min(q.usage_date) into v_unarchived
  from (select distinct (s.started_at at time zone 'Asia/Seoul')::date as usage_date from public.app_usage_sessions s where (s.started_at at time zone 'Asia/Seoul')::date<=v_cutoff) q
  where not public.is_analytics_date_verified_archived('app_usage_sessions',q.usage_date);
  if v_unarchived is not null then raise exception 'raw usage cleanup blocked: % does not have a current VERIFIED archive manifest',v_unarchived; end if;
  for v_day in select distinct (s.started_at at time zone 'Asia/Seoul')::date from public.app_usage_sessions s where (s.started_at at time zone 'Asia/Seoul')::date<=v_cutoff order by 1 loop
    if not public.is_analytics_date_verified_archived('app_usage_sessions',v_day) then raise exception 'raw usage cleanup blocked at delete time: % archive is stale or unverified',v_day; end if;
