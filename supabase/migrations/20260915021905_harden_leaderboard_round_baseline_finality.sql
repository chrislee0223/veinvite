...
begin
  ...
  select count(*)::integer,
    encode(extensions.digest(convert_to(coalesce(string_agg(concat_ws('|',rank_position::text,wallet_address,completed_referrals::text,total_reward_wei::text,reached_count_block::text,reached_count_tx_index::text,reached_count_clause_index::text,reached_count_tx_id), E'\n' order by rank_position),''),'UTF8'),'sha256'),'hex')
  into v_expected_count, v_expected_hash
  from public.get_lifetime_paid_referral_ranking_v2_internal(v_network,p_round_end_block);

  insert into public.leaderboard_round_snapshots(network,round_id,round_end_block,source_checked_through_block,ranking_algorithm_version,row_count,content_sha256)
  values (v_network,p_round_id,p_round_end_block,p_round_end_block,p_ranking_algorithm_version,v_expected_count,v_expected_hash)
  returning id,published_at into v_snapshot_id,v_published_at;

  insert into public.leaderboard_round_snapshot_rows(snapshot_id,wallet_address,rank_position,completed_referrals,total_reward_wei,reached_count_block,reached_count_tx_id,reached_count_tx_index,reached_count_clause_index)
  select v_snapshot_id,wallet_address,rank_position,completed_referrals,total_reward_wei,reached_count_block,reached_count_tx_id,reached_count_tx_index,reached_count_clause_index
  from public.get_lifetime_paid_referral_ranking_v2_internal(v_network,p_round_end_block);
  ...
  return jsonb_build_object('network',v_network,'roundId',p_round_id,'roundEndBlock',p_round_end_block,'rankingAlgorithmVersion',p_ranking_algorithm_version,'ready',true,'created',true,'snapshotId',v_snapshot_id,'rowCount',v_expected_count,'publishedAt',v_published_at);
end;
$function$;
