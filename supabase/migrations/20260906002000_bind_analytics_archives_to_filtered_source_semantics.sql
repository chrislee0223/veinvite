-- Bind analytics archives to the analytics-authoritative source rows.
-- Operator/admin visitors are deliberately excluded from analytics, so they
-- must not be preserved inside the long-term analytics archive either.
-- A late exclusion after verification invalidates the prior archive until a
-- new filtered artifact is exported and verified.

alter table public.veinvite_archive_manifests
  add constraint veinvite_archive_analytics_source_filter_check
  check (
    dataset_key not in ('app_usage_sessions', 'app_product_events')
    or metadata @> '{"sourceFilter":"exclude_analytics_excluded_visitors_v1"}'::jsonb
  ) not valid;

alter table public.veinvite_archive_manifests
  validate constraint veinvite_archive_analytics_source_filter_check;
