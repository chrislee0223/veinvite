-- Production rollout bridge: install the atomic wallet-session RPC before the
-- application starts depending on it. This migration deliberately adds no
-- uniqueness constraints, so the currently deployed challenge route remains
-- fully compatible until the new application code is live.
