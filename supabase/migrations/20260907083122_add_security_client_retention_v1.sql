-- VeInvite Security Client retention v1
--
-- Technical browser/client relationship evidence is intentionally time-bounded.
-- Final eligibility, Sybil decisions, reward accounting, and append-only audit
-- history are not deleted here.

begin;

create table if not exists public.security_client_retention_runs (
  id bigint generated by default as identity primary key,
  policy_version text not null default 'security_client_retention_v1',
  ...
  candidate_clients integer not null check (candidate_clients >= 0),
  deferred_open_review_clients integer not null check (deferred_open_review_clients >= 0),
  deleted_observations integer not null check (deleted_observations >= 0),
  deleted_clients integer not null check (deleted_clients >= 0),
  trigger_source text not null check (
    trigger_source in ('VERCEL_CRON','MANUAL_OPERATOR','SYSTEM_TEST')
  ),
  ran_at timestamptz not null default now()
);

alter table public.security_client_retention_runs enable row level security;
revoke all on table public.security_client_retention_runs
  from public, anon, authenticated;
grant select on table public.security_client_retention_runs to service_role;
grant usage, select on sequence public.security_client_retention_runs_id_seq
  to service_role;

drop trigger if exists security_client_retention_runs_append_only
  on public.security_client_retention_runs;
create trigger security_client_retention_runs_append_only
...
begin
  ...
  perform pg_advisory_xact_lock(
    hashtextextended('veinvite_security_client_retention_v1', 0)
  );

  -- An unresolved Security Client review keeps its technical relationship
  -- evidence even after the normal retention window until an operator resolves
  -- the case. This avoids deleting evidence underneath a live review.
    ...
    and exists (
      ...
      where o.client_id = c.id
        ...
        and (
          ...
          or i.identity_link_status in ('REVIEW','UNKNOWN')
        )
    );
