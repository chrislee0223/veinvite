-- Record successful hot-source cleanup explicitly. A VERIFIED archive whose
-- raw rows were intentionally removed must remain authoritative; otherwise a
-- later maintenance pass could mistake the zero hot-row count for data loss and
-- overwrite permanent rollups. A late row after purge invalidates the sealed
-- archive again.

alter table public.veinvite_archive_manifest_events
  drop constraint veinvite_archive_manifest_events_status_check;

alter table public.veinvite_archive_manifest_events
  add constraint veinvite_archive_manifest_events_status_check
  check (status in (
    'PREPARED','UPLOADED','VERIFIED','HOT_SOURCE_PURGED','FAILED','REVOKED'
  ));
...
declare
  ...
  v_physical_rows bigint;
begin
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'veinvite_archive_manifest:' || new.manifest_id::text,
      0
    )
  );

  select m.dataset_key, m.period_start, m.period_end, m.source_row_count
    into v_dataset_key, v_period_start, v_period_end, v_manifest_source_count
  ...
  if v_previous_status = 'PREPARED'
     and new.status not in ('UPLOADED','FAILED','REVOKED') then
    raise exception 'invalid archive manifest transition: PREPARED -> %', new.status;
  elsif v_previous_status = 'UPLOADED'
     and new.status not in ('VERIFIED','FAILED','REVOKED') then
    raise exception 'invalid archive manifest transition: UPLOADED -> %', new.status;
  elsif v_previous_status = 'VERIFIED'
     and new.status not in ('HOT_SOURCE_PURGED','REVOKED') then
    raise exception 'invalid archive manifest transition: VERIFIED -> %', new.status;
  elsif v_previous_status = 'HOT_SOURCE_PURGED'
     and new.status <> 'REVOKED' then
    raise exception 'invalid archive manifest transition: HOT_SOURCE_PURGED -> %', new.status;
  ...
  if new.status = 'VERIFIED'
     and not (
       new.details @> '{"artifactChecksumVerified":true,"sourceRowCountVerified":true}'::jsonb
     ) then
  ...
  if new.status = 'HOT_SOURCE_PURGED' then
    if not (new.details @> '{"sourceRowsPurged":true}'::jsonb)
      ...
      raise exception 'HOT_SOURCE_PURGED requires sourceRowsPurged, archivedRowCount and physicalRowsDeleted';
    end if;

    if (new.details->>'archivedRowCount')::bigint <> v_manifest_source_count then
      raise exception 'HOT_SOURCE_PURGED archivedRowCount must equal manifest source_row_count';
    end if;

    if (new.details->>'physicalRowsDeleted')::bigint < v_manifest_source_count then
      raise exception 'HOT_SOURCE_PURGED physicalRowsDeleted cannot be smaller than archivedRowCount';
    end if;

    if v_dataset_key = 'app_usage_sessions' then
      select count(*) into v_physical_rows
      from public.app_usage_sessions s
      where (s.started_at at time zone 'Asia/Seoul')::date
            between v_period_start and v_period_end;
    elsif v_dataset_key = 'app_product_events' then
      select count(*) into v_physical_rows
      from public.app_product_events e
      where e.usage_date between v_period_start and v_period_end;
    else
      raise exception 'HOT_SOURCE_PURGED is only valid for raw analytics datasets';
    end if;

    if v_physical_rows <> 0 then
      raise exception 'HOT_SOURCE_PURGED cannot be recorded while % physical source rows remain', v_physical_rows;
    end if;
  end if;
...
revoke all on function public.enforce_archive_manifest_event_transition()
  from public, anon, authenticated;
grant execute on function public.enforce_archive_manifest_event_transition()
  to postgres, service_role;

create or replace function public.current_verified_analytics_archive_manifest_id(
  p_dataset_key text,
  p_usage_date date
)
returns bigint
  ...
  loop
    ...
    if v_current_count = v_manifest.source_row_count
       and (
         ...
         or v_latest_source_change <= v_manifest.verified_at
       ) then
...
revoke all on function public.current_verified_analytics_archive_manifest_id(text,date)
  from public, anon, authenticated;
grant execute on function public.current_verified_analytics_archive_manifest_id(text,date)
  to postgres, service_role;
