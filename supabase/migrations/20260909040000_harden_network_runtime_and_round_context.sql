create table if not exists public.network_runtime_config (
  id smallint primary key,
  enabled boolean not null default true,
  note text,
  updated_at timestamptz not null default now(),
  constraint network_runtime_config_singleton_check check (id = 1)
);

insert into public.network_runtime_config (id, enabled, note)
values (1, true, 'Network canvas runtime kill switch')
on conflict (id) do nothing;

alter table public.network_runtime_config enable row level security;

revoke all on table public.network_runtime_config from public, anon, authenticated;
grant select, insert, update on table public.network_runtime_config to service_role;

drop policy if exists network_runtime_config_service_role on public.network_runtime_config;
create policy network_runtime_config_service_role
...
comment on table public.network_runtime_config is
  'Service-role-only runtime controls for the VeInvite Network surface. Set enabled=false to fail closed without a redeploy.';

create or replace function public.read_referral_network_focus_v2(
  p_root_wallet text,
...
branch_metrics as (
  select
    ...
    (
      ...
      + case when dr.member_status in ('QUALIFIED', 'REWARDED') then 1 else 0 end
    ...
    case
      when p_round_id is null or p_round_start_at is null then null::integer
        ...
        count(fn.wallet) filter (
          where fn.depth > 1
            and fn.joined_at >= p_round_start_at
            and fn.joined_at < coalesce(least(p_round_end_at, now()), now())
        )
        + case
...
focus_summary as (
  select
    ...
    count(fn.wallet) filter (
      where fn.member_status in ('QUALIFIED', 'REWARDED')
    )::integer as qualified_count,
    case
      when p_round_id is null or p_round_start_at is null then null::integer
      else count(fn.wallet) filter (
        where fn.joined_at is not null
          and fn.joined_at >= p_round_start_at
          and fn.joined_at < coalesce(least(p_round_end_at, now()), now())
      )::integer
    end as round_growth,
...
select
  case
    when not (select enabled from runtime)
      then jsonb_build_object('error', 'NETWORK_DISABLED')
    when not exists (
...
revoke all on function public.read_referral_network_focus_v2(
  text, text, text, bigint, timestamptz, timestamptz
) from public, anon, authenticated;
grant execute on function public.read_referral_network_focus_v2(
  text, text, text, bigint, timestamptz, timestamptz
) to service_role;
...
comment on function public.read_referral_network_focus_v2(
  ...
  'Service-role Network read model with a runtime kill switch, authoritative caller-supplied current-round window, branch-inclusive qualified/growth counts for non-root focus, and sponsor-only topology.';
