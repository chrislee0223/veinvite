-- VeInvite Network is intentionally default-public and read-only.
-- Retire the per-wallet visibility opt-in and derive the visible graph only
-- from verified referral-network data.
...
root_known as (
  select exists (
    select 1
    from public.operator_referral_leaderboard l
    join params p on lower(l.wallet_address) = p.root_wallet
    ...
    join params p
      on lower(e.sponsor_wallet) = p.root_wallet
      or lower(e.child_wallet) = p.root_wallet
  ) as allowed
),
visible_tree as (
  select
  ...
  where p.root_wallet ~ '^0x[0-9a-f]{40}$'
    and (select allowed from root_known)
...
select case
  ...
  when not (select allowed from root_known)
    then jsonb_build_object('error', 'NETWORK_NOT_FOUND')
...
eligible_roots as (
  select
    ...
    l.last_activity_at as updated_at,
    md5(
      lower(l.wallet_address) || ':' ||
  ...
  where exists (
    select 1
    from public.qualified_referral_network_edges e
    where lower(e.sponsor_wallet) = lower(l.wallet_address)
  )
    and (
      ...
      or (
        ...
        and exists (
          select 1
          from public.network_runtime_canary_wallets cw
          where cw.wallet_address = lower(l.wallet_address)
        )
      )
    )
),
