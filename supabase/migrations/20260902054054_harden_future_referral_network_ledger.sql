alter function public.sync_referral_relationship_from_invitation() security definer;
alter function public.sync_referral_relationship_from_invitation() set search_path to pg_catalog, public;
revoke all on function public.sync_referral_relationship_from_invitation() from public, anon, authenticated;

alter function public.capture_referral_network_snapshot(text) security definer;
alter function public.capture_referral_network_snapshot(text) set search_path to pg_catalog, public;
...
alter table public.referral_relationships
  add constraint referral_relationships_source_snapshot_object_check
  check (jsonb_typeof(source_snapshot) = 'object');

alter table public.referral_network_snapshots
  add constraint referral_network_snapshots_details_object_check
...
begin
  ...
  if old.invitee_wallet is not null
     and lower(btrim(new.invitee_wallet)) is distinct from lower(btrim(old.invitee_wallet)) then
...
revoke all on function public.prevent_invitation_referral_identity_mutation() from public, anon, authenticated;
...
revoke all on function public.prevent_referral_network_snapshot_mutation() from public, anon, authenticated;

drop trigger if exists referral_network_snapshots_append_only_update on public.referral_network_snapshots;
create trigger referral_network_snapshots_append_only_update
before update on public.referral_network_snapshots
for each row execute function public.prevent_referral_network_snapshot_mutation();

drop trigger if exists referral_network_snapshots_append_only_delete on public.referral_network_snapshots;
create trigger referral_network_snapshots_append_only_delete
before delete on public.referral_network_snapshots
for each row execute function public.prevent_referral_network_snapshot_mutation();
