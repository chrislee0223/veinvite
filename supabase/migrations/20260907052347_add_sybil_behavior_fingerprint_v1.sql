...
with ranked_rewards as (
  select
    ...
    row_number() over (
      ...
      order by e.block_number, coalesce(e.tx_index, 2147483647), coalesce(e.clause_index, 2147483647), e.tx_id, e.id
    ) as reward_order,
  ...
  select * from ranked_rewards where reward_order <= 3
), aggregated as (
  select
    r.invite_code,
    ...
    array_agg(extract(epoch from (r.block_timestamp - r.previous_reward_at))::bigint order by r.reward_order)
    ...
    extract(epoch from (max(r.block_timestamp) - min(r.block_timestamp)))::bigint as reward_span_seconds
  from first_three r
  group by r.invite_code
  having count(*) = 3
), sequence_frequency as (
  select a.app_sequence, count(*)::integer as same_sequence_invite_count
  from aggregated a
...
select
  ...
  a.first_reward_at,
  a.third_reward_at,
  extract(epoch from (a.first_reward_at - i.activated_at))::bigint as activation_to_first_reward_seconds,
...
join sequence_frequency f on f.app_sequence = a.app_sequence
...
revoke all on public.operator_sybil_behavior_fingerprints from public, anon, authenticated;
grant select on public.operator_sybil_behavior_fingerprints to service_role;
...
with pairs as (
  select
    ...
    abs(extract(epoch from (a.activated_at - b.activated_at)))::bigint as activation_delta_seconds,
  ...
  select
    ...
    (d.max_relative_delta <= 0.20 and d.max_absolute_delta_seconds <= 300) as similar_timing,
    (p.activation_delta_seconds <= 3600) as clustered_activation,
    (p.vet_funder_a is not null and p.vet_funder_a = p.vet_funder_b) as shared_vet_funder,
    (p.vtho_funder_a is not null and p.vtho_funder_a = p.vtho_funder_b) as shared_vtho_funder
  from pairs p
  cross join lateral (
    select
      max(abs(p.intervals_a[g.i] - p.intervals_b[g.i]))::bigint as max_absolute_delta_seconds,
      max(abs(p.intervals_a[g.i] - p.intervals_b[g.i])::numeric / greatest(p.intervals_a[g.i], p.intervals_b[g.i], 60)) as max_relative_delta
  ...
  select
    ...
    ((case when d.similar_timing then 20 else 0 end)
      ...
      + (case when d.shared_vtho_funder then 3 else 0 end))::integer as observation_score
  from deltas d
)
select
  s.network,
  ...
  0::integer as sequence_only_score,
  s.observation_score,
  case when s.similar_timing and s.clustered_activation and s.shared_funder then 'WATCH' else 'INFO' end::text as attention_level,
  'behavior-v1'::text as fingerprint_version
from scored s;
...
revoke all on public.operator_sybil_behavior_similarity_candidates from public, anon, authenticated;
grant select on public.operator_sybil_behavior_similarity_candidates to service_role;
