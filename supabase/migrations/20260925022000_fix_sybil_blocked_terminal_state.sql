...
begin
  if old.status <> 'CANCELLED'
    ...
    if not (
      ...
      and new.sybil_reason is not null
      and length(btrim(new.sybil_reason)) > 0
      and old.reward_status <> 'PAID'
      and new.reward_status <> 'PAID'
      and not exists (
        select 1
...
declare
  v_def text;
  v_old text := 'where i.status = ''UNDER_REVIEW''';
  v_new text := 'where i.status = ''UNDER_REVIEW''' || chr(10) ||
                '        and i.sybil_status = ''REVIEW''';
  v_count integer;
begin
  select pg_get_functiondef(
    'public.record_operator_anomaly_snapshot(text,text)'::regprocedure
  )
  into v_def;
  ...
  if v_count <> 2 then
    raise exception 'OPERATOR_MONITOR_UNDER_REVIEW_FILTER_COUNT_MISMATCH:%', v_count;
  end if;
