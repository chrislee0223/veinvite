...
alter table public.security_client_retention_runs
  drop constraint security_client_retention_runs_retention_days_check;

alter table public.security_client_retention_runs
  add constraint security_client_retention_runs_retention_days_check
...
begin
  ...
  if p_retention_days is distinct from 365 then
    raise exception 'security client retention days are fixed at 365';
  end if;
  ...
  select count(distinct c.id)::integer into v_deferred
  from public.security_clients c
  where c.created_at < v_cutoff
    and exists (
      select 1
      from public.security_client_wallet_observations o
      join public.invitations i on lower(btrim(i.invitee_wallet)) = o.wallet_address
      where o.client_id = c.id
        and i.status = 'UNDER_REVIEW'
        and i.sybil_status = 'REVIEW'
        and (i.sybil_source = 'SECURITY_CLIENT' or i.identity_link_status = 'REVIEW')
    );

  select coalesce(array_agg(s.id), array[]::uuid[]) into v_client_ids
  from (
    select c.id
    from public.security_clients c
    where c.created_at < v_cutoff
  ...
  if v_candidate_clients > 0 then
    delete from public.security_client_wallet_observations o where o.client_id = any(v_client_ids);
    get diagnostics v_deleted_observations = row_count;
    delete from public.security_clients c where c.id = any(v_client_ids);
    get diagnostics v_deleted_clients = row_count;
  end if;

  insert into public.security_client_retention_runs(
    policy_version, retention_days, batch_limit, cutoff_at, candidate_clients,
    deferred_open_review_clients, deleted_observations, deleted_clients,
    trigger_source, ran_at
  ) values (
    'security_client_retention_v1', p_retention_days, p_batch_limit, v_cutoff,
    v_candidate_clients, v_deferred, v_deleted_observations, v_deleted_clients,
    p_trigger_source, v_now
  );
...
revoke all on function public.cleanup_security_client_identity_data(text,integer,integer)
  from public, anon, authenticated;
grant execute on function public.cleanup_security_client_identity_data(text,integer,integer)
  to service_role;
