...
begin
  ...
  -- A current HOLD review belongs to the inviter posture, not to UUID ordering.
  -- Closely grouped restrictions can share the same timestamp, so requiring the
  -- review view's latest random UUID to equal NEW.id can suppress the transition
  -- notification even though the inviter genuinely entered HOLD.
  select exists (
    select 1
