-- Require a verified B3TR -> VOT3 conversion before a governance vote can
-- make a VeInvite referral reward-eligible.
--
-- Policy:
-- - at least one qualifying dApp reward must happen after invite activation;
-- - after that first dApp reward, the invitee must convert >= 1 B3TR to VOT3;
-- - the allocation vote must happen no earlier than that conversion;
-- - the other two dApp rewards may happen before OR after conversion/vote;
-- - old VOT3 balances and VOT3 received by transfer do not count.

begin;

alter table public.invitations
  add column if not exists vot3_converted boolean not null default false,
  add column if not exists vot3_converted_at timestamptz,
  add column if not exists vot3_converted_block bigint,
  add column if not exists vot3_conversion_tx_id text,
  add column if not exists vot3_conversion_amount_wei text;
...
alter table public.invitations
  ...
  check (
    ...
    (
      ...
      and vot3_conversion_amount_wei::numeric >= 1000000000000000000
    )
  );

alter table public.invite_impact_events
  drop constraint if exists invite_impact_events_type_check,
  drop constraint if exists invite_impact_events_shape_check;

alter table public.invite_impact_events
  add constraint invite_impact_events_type_check
  check (
    event_type in (
      'DAPP_REWARD',
      'VOT3_CONVERSION',
...
create index if not exists invite_impact_events_conversion_idx
  ...
  where event_type = 'VOT3_CONVERSION';
...
begin
  ...
  -- The three-dApp checkpoint describes only the third distinct rewarded app.
  -- It must NOT clear an earlier valid conversion/vote sequence: users are
  -- allowed to convert/vote after dApp #1 and finish dApps #2/#3 later.
  ...
  -- A qualifying conversion is independently anchored to invite activation.
  -- The raw-impact check below additionally proves that at least one dApp
  -- reward happened no later than this conversion.
  if coalesce(new.vot3_converted, false) = false
     ...
     or new.vot3_conversion_amount_wei::numeric < 1000000000000000000 then
    ...
    -- A vote cannot be a VeInvite mission vote without a qualifying conversion.
    new.vote_completed := false;
    new.vote_completed_at := null;
    new.vote_completed_block := null;
    new.vote_round_id := null;
  else
    -- Votes may be before the third dApp reward, but never before conversion.
  ...
  if new.status = 'COMPLETED'
     ...
     and new.vot3_conversion_amount_wei::numeric >= 1000000000000000000
...
-- Re-open modern, unpaid rows that were considered complete under the old
-- dApp->vote-only proof model. Legacy rows remain untouched. The next normal
-- reconciliation pass will backfill valid historical-in-this-invite conversion
-- and vote evidence from chain truth.
update public.invitations
set
  impact_sync_complete_at = null,
  status = case
    when status = 'COMPLETED' then 'ACTIVATING'
    else status
  end
...
with impact_summary as (
  select
    ...
    ) as first_dapp_reward_block,
    count(distinct app_id) filter (
      where event_type = 'DAPP_REWARD'
    ) as distinct_dapp_reward_count,
    count(*) filter (
      where event_type = 'VOT3_CONVERSION'
    ) as conversion_event_count,
    max(amount_wei::numeric) filter (
      where event_type = 'VOT3_CONVERSION'
        and amount_wei ~ '^[0-9]+$'
    ) as max_conversion_amount_wei,
    min(block_number) filter (
      where event_type = 'VOT3_CONVERSION'
        and amount_wei ~ '^[0-9]+$'
        and amount_wei::numeric >= 1000000000000000000
    ) as first_minimum_conversion_block
...
select
  ...
  i.apps_completed,
  i.rewards_received,
  s.first_dapp_reward_block,
  coalesce(s.distinct_dapp_reward_count, 0) as distinct_dapp_reward_count,
  i.vot3_converted,
  i.vot3_converted_at,
  i.vot3_converted_block,
  i.vot3_conversion_tx_id,
  i.vot3_conversion_amount_wei,
  coalesce(s.conversion_event_count, 0) as conversion_event_count,
  ...
  i.sybil_risk_level,
  i.sybil_reason,
  i.impact_sync_complete_at,
  case
    when i.invitee_wallet is null then 'NOT_CLAIMED'
    when i.status = 'CANCELLED' or i.reward_status = 'FORFEITED'
      then 'FORFEITED_OR_CANCELLED'
    when coalesce(i.apps_completed, 0) = 0
      then 'NEED_FIRST_DAPP_REWARD'
    when coalesce(i.vot3_converted, false) = false
         and s.first_minimum_conversion_block is not null
         and s.first_dapp_reward_block is not null
         and s.first_minimum_conversion_block < s.first_dapp_reward_block
      then 'VOT3_CONVERSION_BEFORE_FIRST_DAPP'
    when coalesce(i.vot3_converted, false) = false
         and coalesce(s.conversion_event_count, 0) > 0
         and coalesce(s.max_conversion_amount_wei, 0) < 1000000000000000000
      then 'CONVERSION_AMOUNT_TOO_LOW'
    when coalesce(i.vot3_converted, false) = false
      then 'NEED_VOT3_CONVERSION'
    when coalesce(i.vote_completed, false) = false
      then 'NEED_VOTE_AFTER_CONVERSION'
    when coalesce(i.apps_completed, 0) < 3
      then 'NEED_MORE_DAPP_REWARDS'
    when i.sybil_status = 'BLOCKED'
      then 'SYBIL_BLOCKED'
    when i.sybil_status = 'REVIEW' or i.status = 'UNDER_REVIEW'
      then 'SYBIL_REVIEW'
    when i.reward_status = 'PAID'
      then 'PAID'
    when i.reward_status = 'ELIGIBLE'
      then 'ELIGIBLE'
    when i.status = 'COMPLETED'
      then 'PENDING_REWARD_SETTLEMENT'
...
left join impact_summary s
  on s.invite_code = i.invite_code;

revoke all on public.operator_invitee_mission_diagnostics
  from public, anon, authenticated;
grant select on public.operator_invitee_mission_diagnostics
  to service_role;
