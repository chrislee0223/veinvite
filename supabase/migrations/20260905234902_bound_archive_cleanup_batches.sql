alter table public.veinvite_archive_manifests
  add constraint veinvite_archive_analytics_daily_period_check
  check (
    dataset_key not in ('app_usage_sessions', 'app_product_events')
    or period_start = period_end
  ) not valid;

alter table public.veinvite_archive_manifests
  validate constraint veinvite_archive_analytics_daily_period_check;
...
begin
  ...
  -- Destructive cleanup is intentionally bounded to one Seoul calendar day per
  -- transaction. Lock both raw analytics sources in the same order as usage
  -- cleanup so the final rollups see one stable snapshot and ingestion is
  -- blocked only for a short, predictable interval.
...
begin
  ...
  select min((s.started_at at time zone 'Asia/Seoul')::date)
    into v_day
  from public.app_usage_sessions s
  ...
  -- Product events must compact first while daily exclusion markers still
  -- exist, otherwise a later product rollup could accidentally include an
  -- excluded operator/admin visitor.
  if exists (
    select 1
    from public.app_product_events e
