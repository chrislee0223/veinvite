-- Preview-validated hardening for long-term analytics recalculation.
-- 1. Rebuild product rollups atomically per day so stale combinations cannot survive.
-- 2. Re-finalize completed historical days when a visitor is later excluded.
-- 3. Preserve a deterministic all/all zero funnel row for completed zero-visitor days.
-- 4. Never erase permanent rollups solely because raw data was already VERIFIED archived.
...
begin
  ...
  from public.app_product_events e cross join lateral(values('locale'::text,e.locale),('device'::text,e.device_bucket),('source'::text,e.acquisition_source)) d(dimension_name,dimension_value)
  where e.usage_date=p_usage_date and not exists(select 1 from public.app_usage_excluded_visitors x where x.visitor_key=e.visitor_key)
...
begin
  ...
  select coalesce(array_agg(d.usage_date order by d.usage_date),'{}'::date[]) into v_affected_days from (
    ...
    union select distinct e.usage_date from public.app_product_events e where e.visitor_key=p_visitor_key and e.usage_date<v_today
  ) d;
  insert into public.app_usage_excluded_visitors(visitor_key,excluded_at,reason) values(p_visitor_key,clock_timestamp(),'ADMIN_WALLET') on conflict(visitor_key) do nothing; v_inserted:=found;
