...
begin
  ...
  -- A previous-round baseline is only needed once the current round has a
  -- finalized paid result. Waiting for a post-boundary settlement also ensures
  -- the payout worker has advanced beyond any older pre-boundary transaction
  -- before we freeze the historical paid-referral ranking.
  if not exists (
    select 1
