...
where not public.is_sybil_v2_referral_invalidated(
  q.source_invite_code,
  q.resolved_network
);
...
where i.status = 'COMPLETED'
  and i.reward_status = any(array['ELIGIBLE'::text,'PAID'::text])
...
select
  ...
  count(*) filter (
    where i.status = 'COMPLETED'
      ...
      and coalesce(e.entry_class, legacy.entry_class)
        = any(array['NEW'::text,'RETURNING'::text])
  ...
  count(*) filter (
    where i.sybil_status = any(array['REVIEW'::text,'BLOCKED'::text])
  ...
  select
    (
      ...
      and not exists (
        ...
        where x.network = lower(btrim(p_network))
          and x.status = 'ACTIVE'
          and x.decided_at > s.published_at
      )
...
begin
  ...
  country_totals as (
    select
      ...
      count(*) filter(where q.entry_class='RETURNING')::bigint
        as returning_users,
      count(*) filter(where q.cohort_round_id=p_current_round_id)::bigint
        as current_round_completed
    from qualified q
    ...
    'unknownCurrentRoundCompleted',
      (select count(*) from qualified
       where country_code is null
         and cohort_round_id=p_current_round_id),
    'leaders',
      coalesce((
        select jsonb_agg(
          jsonb_build_object(
...
select
  l.entry_class,
  coalesce(l.display_language,'unknown'::text) as display_language,
  count(*) as participant_count,
  count(*) filter(where l.has_saved_preference) as saved_preference_count,
  count(*) filter(where l.display_language_source='browser_auto')
    as browser_auto_count,
  count(*) filter(where l.display_language_source='local_storage')
    as local_storage_count,
  count(*) filter(where l.display_language_source='wallet_preference')
    as wallet_preference_count,
  count(*) filter(where l.display_language_source='manual_selection')
    as manual_selection_count,
  count(*) filter(where l.display_language_source is null)
    as unknown_source_count,
  count(*) filter(where l.display_language_source='query_param')
    as query_param_count
from public.operator_accepted_wallet_languages l
join public.invitations i
  on i.id = l.invitation_id
where not public.is_sybil_v2_referral_invalidated(
  i.invite_code,
  i.activation_network
)
group by
  l.entry_class,
  coalesce(l.display_language,'unknown'::text)
order by
  l.entry_class,
  count(*) desc,
  coalesce(l.display_language,'unknown'::text);
