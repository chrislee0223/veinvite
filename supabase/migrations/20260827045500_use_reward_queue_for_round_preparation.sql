...
begin
  ...
  -- One preparer per VeInvite network/app at a time. The lock is released
  ...
  -- Unsettled payouts are already promised from the observable pool and must
  -- remain reserved before a new round can be created.
  ...
  -- Lock both queue and invitation rows while taking the settlement snapshot.
  -- Every candidate is revalidated against current invitation state and raw
  -- evidence so a stale queue row cannot be settled accidentally.
  select coalesce(
...
-- Queue entries may be re-queued only before assignment. Once a settlement
-- snapshot assigns an entry, its financial identity and round are immutable.
...
-- Payout identity and amount are fixed when the round is prepared. Retry
-- processing may update status/attempt/error/transaction metadata only.
