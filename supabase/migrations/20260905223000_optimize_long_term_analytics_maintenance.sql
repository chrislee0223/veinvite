...
begin
  ...
  -- Refresh only the last three completed days to absorb recent session
  -- updates. Older retained raw dates are processed only when a permanent
  -- rollup is missing, keeping daily work bounded as the 365-day hot window
  -- grows.
  ...
  for v_day in
    ...
    where d.usage_date >= v_recent_from
       ...
       or not exists (
         select 1
         from public.veinvite_daily_funnel_rollups f
