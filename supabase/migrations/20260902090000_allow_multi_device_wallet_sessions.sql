drop index if exists public.wallet_auth_sessions_one_unrevoked_per_wallet_idx;

create index if not exists wallet_auth_sessions_active_wallet_created_idx
on public.wallet_auth_sessions (wallet_address, created_at desc, id desc)
...
begin
  ...
  -- Expired sessions do not need to remain logically active. Mark them revoked
  -- before ranking the live sessions so the per-wallet cap only counts usable
  ...
  -- Keep up to five independent active browser/device sessions for one wallet.
  -- Signing in on a second device must not invalidate the first device. The
  -- advisory lock above serializes concurrent issuance for the same wallet.
  ...
  update public.wallet_auth_sessions as sessions
  set revoked_at = p_used_at
  from ranked_sessions
  where sessions.id = ranked_sessions.id
    and ranked_sessions.session_rank > 5;
