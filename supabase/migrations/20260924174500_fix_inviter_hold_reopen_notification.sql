...
begin
  ...
  select exists (
    ...
    where c.network = new.network
      ...
      and c.latest_incident_id = new.id
  )
...
comment on function public.notify_sybil_v2_inviter_incident_history() is
  'Creates inviter WATCH/HOLD history only for meaningful escalation transitions. An unresolved HOLD is not repeated, but a new incident after operator CLEAR creates a fresh unresolved HOLD snapshot and notification.';
