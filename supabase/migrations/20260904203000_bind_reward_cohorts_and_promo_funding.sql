...
-- A participant belongs to the operating round in which onboarding begins.
-- The funding receipt is the previous VeBetter allocation round that finances
-- that cohort (e.g. allocation round 113 -> onboarding cohort 114).
...
-- Non-official funding is deliberately separated from VeBetter allocation
-- evidence so promo deposits never contaminate official allocation history.
...
revoke all on sequence public.reward_cohort_funding_adjustments_id_seq
  from public, anon, authenticated;
grant select, insert on table public.reward_cohort_funding_adjustments
  to service_role;
grant usage, select on sequence public.reward_cohort_funding_adjustments_id_seq
  to service_role;
...
revoke all on function public.validate_reward_cohort_funding_adjustment()
  from public, anon, authenticated;
grant execute on function public.validate_reward_cohort_funding_adjustment()
  to service_role;
...
revoke all on function public.reject_reward_cohort_funding_adjustment_mutation()
  from public, anon, authenticated;
grant execute on function public.reject_reward_cohort_funding_adjustment_mutation()
  to service_role;
...
-- Bind the cohort automatically inside the same atomic claim transaction.
-- currentRoundId comes from the sealed VeBetter entry-eligibility evidence.
...
begin
  ...
  if new.activated_at is not null
    ...
    elsif new.activation_network = 'mainnet' then
      raise exception 'REWARD_COHORT_FUNDING_NOT_SYNCED';
    end if;
  end if;
...
drop trigger if exists invitations_bind_reward_cohort on public.invitations;
create trigger invitations_bind_reward_cohort
before insert or update on public.invitations
for each row execute function public.bind_invitation_reward_cohort();

-- The first funded VeInvite cohort is round 114. Existing genuine active
-- referrals were already onboarding when the round-113 allocation arrived, so
-- they are explicitly attached to the inaugural funded cohort.
with funding as (
  select a.id
  from public.vebetter_round_allocations a
...
-- One-off Round 114 launch promotion. The amount is the observed 2026-09-04
-- reward-pool increase from 886.44 to 2,103.77 B3TR. It is intentionally not
-- written to vebetter_round_allocations and therefore cannot pollute official
-- allocation-history learning.
...
-- Snapshot a single cohort's funding and remaining pipeline. Global reserved
-- liability protects the physical pool; cohort reserved liability protects the
-- designated round budget from being spent by another cohort.
...
begin
  ...
  if v_receipt.id is null then
    return jsonb_build_object(
      ...
      'designatedBudgetWei','0',
      'rewardCohortRoundId',null,
      'pipeline',jsonb_build_object(
        'queuedEligibleCount',0,'voteReadyCount',0,'vot3ReadyCount',0,
        'appsTwoCount',0,'appsOneCount',0,'activatedZeroCount',0,'pendingAcceptanceCount',0
      ),
  ...
  return jsonb_build_object(
    ...
    'latestAllocation', jsonb_build_object(
      'id',v_receipt.id,
      'veBetterRoundId',v_receipt.vebetter_round_id,
...
-- Defense in depth: even a stale or buggy server quote cannot reserve more than
-- the funding explicitly designated for the invitation's cohort.
...
begin
  if new.reserved_amount_wei is null then
    return new;
  end if;
...
-- Cohort-scoped batch preparation. Fixed invitation reservations remain the
-- payout authority; this function only batches claimed reservations that share
-- the exact same funding receipt, preserving accounting provenance end-to-end.
