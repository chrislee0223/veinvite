...
if (!/@media \(max-width:420px\)[\s\S]*gap:6px\s*!important[\s\S]*flex-basis:18px\s*!important/.test(layoutPolish)) {
  failures.push('Reviewed 420px inviter identity spacing is missing.');
}
if (!/@media \(max-width:360px\)[\s\S]*gap:5px\s*!important[\s\S]*flex-basis:16px\s*!important/.test(layoutPolish)) {
  failures.push('Reviewed 360px inviter identity spacing is missing.');
}
