...
if (/get_public_lifetime_leaderboard['"]/.test(route)) {
  failures.push('Leaderboard fallback must not call the retired legacy lifetime leaderboard RPC.');
}
if (
  !/const readLeaderboardWithoutMovement = \(\) =>[\s\S]*get_public_lifetime_leaderboard_v2[\s\S]*p_comparison_round_id:\s*null[\s\S]*p_ranking_algorithm_version:\s*RANKING_ALGORITHM_VERSION/.test(route) ||
  !/fallbackResult = await readLeaderboardWithoutMovement\(\)/.test(route)
) {
  failures.push('Leaderboard movement failures must fall back to the authoritative v2 ranking with movement disabled.');
}
