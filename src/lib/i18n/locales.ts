...
export const SUPPORTED_LOCALES = LOCALE_DEFINITIONS.map((definition) => definition.locale);
export type SupportedLocale = (typeof LOCALE_DEFINITIONS)[number]['locale'];
export type Locale = string;
export const LANGUAGE_STORAGE_KEY = 'veinvite-language';
export type LanguageOption = { locale: SupportedLocale; nativeName: string; flagSource: string; direction: LocaleDirection; typography: LocaleTypography; };
export const LANGUAGE_OPTIONS: LanguageOption[] = LOCALE_DEFINITIONS.map((definition) => ({ ...definition }));
const SUPPORTED_LOCALE_SET = new Set<string>(SUPPORTED_LOCALES);
export function isLocale(value: unknown): value is SupportedLocale { return typeof value === 'string' && SUPPORTED_LOCALE_SET.has(value); }
export function localeFromLanguageTag(value: string | null | undefined): SupportedLocale | null {
...
export function resolveBrowserLocale(languages: readonly string[] | undefined, fallback: SupportedLocale = 'en'): SupportedLocale {
  for (const language of languages ?? []) { const locale = localeFromLanguageTag(language); if (locale) return locale; }
  return fallback;
}
export function getLanguageOption(locale: string): LanguageOption { return LANGUAGE_OPTIONS.find((option) => option.locale === locale) ?? LANGUAGE_OPTIONS[0]; }
export function getLocaleDirection(locale: string): LocaleDirection { return getLanguageOption(locale).direction; }
export function getLocaleTypography(locale: string): LocaleTypography { return getLanguageOption(locale).typography; }
export function isRtlLocale(locale: string): boolean { return getLocaleDirection(locale) === 'rtl'; }
export function isCjkLocale(locale: string): boolean { return getLocaleTypography(locale) === 'cjk'; }
