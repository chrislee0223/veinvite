...
export async function POST(
  ...
  const source =
    typeof record.source === 'string'
      ? record.source
      : '';
  const activeDeltaSeconds = Number(
    record.activeDeltaSeconds,
  );
  const returningVisitor =
    record.returningVisitor;
  ...
  if (
    ...
    typeof returningVisitor !== 'boolean'
  ) {
    return noStore(400);
  }

  const hashedVisitor = visitorKey(
    rawVisitorId,
  );
  const ipSubject =
  ...
  if (error) {
    console.error(
      'Usage analytics event could not be stored:',
      error,
    );
    return noStore(503);
  }
