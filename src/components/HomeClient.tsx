...
export function HomeClient() {
  ...
  const active =
    latest &&
    ['PENDING_ACCEPTANCE', 'ACTIVATING', 'UNDER_REVIEW'].includes(latest.status)
      ? latest
      : undefined;
  const completed = latest?.status === 'COMPLETED' ? latest : undefined;

  const waitingForFriend =
    active?.status === 'PENDING_ACCEPTANCE' && !active.inviteeAddress;
  ...
  const statusLabel = waitingForFriend
    ? '친구 참여 대기'
    : activating
      ? '활성화 진행 중'
      : underReview
  ...
  const heroTitle = waitingForFriend
    ? '초대 링크가 준비됐어요'
    : activating
      ? '친구가 초대를 수락했어요'
      : underReview
  ...
  const heroDescription = waitingForFriend
  ...
  const panelDescription = waitingForFriend
  ...
  const cancelDescription = waitingForFriend
  ...
  return (
    <main className="appShell">
      ...
      {!wallet ? (
        <div className="notice">먼저 지갑을 연결해 주세요.</div>
      ) : null}
      ...
      {active ? (
        <section className="panel">
          ...
          <div className="linkBox">
            <code>{inviteUrl || `/i/${active.code}`}</code>
          </div>

          <div className="buttonGrid">
            <button className="secondaryButton" onClick={copyInvite}>
              링크 복사
            </button>
            <button className="primaryButton inline" onClick={shareInvite}>
              공유하기
            </button>
          </div>
          ...
          {active.inviteeAddress ? (
            <p className="walletLine">
              연결 지갑: {active.inviteeAddress.slice(0, 8)}···
              {active.inviteeAddress.slice(-6)}
            </p>
          ) : null}
          ...
          >
            {waitingForFriend ? '초대 링크 취소하기' : '초대 취소하기'}
          </button>
        </section>
      ) : (
      ...
      {completed ? (
        <section className="panel rewardPanel">
          <div>
            <span className="eyebrow">이번 주 추천 보상</span>
            <h2>정산 참여 중</h2>
            <p className="muted">
              주간 참여 결과에 따라 금액이 결정됩니다.
            </p>
          </div>
      ...
      {showCancel ? (
        <div className="modalBackdrop" role="dialog" aria-modal="true">
          <div className="modalCard">
            ...
            <button className="dangerLink" onClick={cancelInvite}>
              {waitingForFriend ? '초대 링크 취소' : '초대 취소'}
            </button>
          </div>
        </div>
      ) : null}
