  ...
  const claimReward = async (invite: InviteRecord) => {
    ...
    try {
      ...
      if (failureCode !== 'wallet_auth') {
        const reconciliation = await reconcileRewardClaimState(invite.code);
        if (!sameWallet(activeWalletRef.current, requestWallet)) return;
  ...
  return (
    <main className={`screen${activeTab === 'guide' ? ' networkScreen' : ''}`}>
      ...
      <style jsx>{`
        ...
        .screen.networkScreen { width:min(100%,548px); height:100svh; min-height:100svh; margin:0 auto; overflow:hidden; overscroll-behavior:none; padding:14px 14px calc(72px + env(safe-area-inset-bottom)); display:flex; flex-direction:column; }
