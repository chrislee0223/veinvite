...
export function PublicLeaderboardHub({
  ...
  // Keep the existing component as the loading/error fallback so the current
  // leaderboard behavior remains intact even if the new aggregate view cannot
  // initialize on a particular client.
  if (!data || state.failed) {
    return <PublicLeaderboard locale={locale} wallet={wallet} />;
  }
  ...
  return (
    <section className="leaderboardHub">
        ...
        <div className="rankingMeta" aria-live="polite">
          {rankingView === 'inviter' ? (
            ...
            <>
              ...
              <span>
                {copy.unknown} {unknownCompleted.toLocaleString()}
              </span>
            </>
          )}
        </div>
      ...
      <style jsx>{`
        ...
        .countryScroll::-webkit-scrollbar-track {
          background:transparent;
        }
