...
export function AppNetwork({
  ...
  }, [wallet, focusWallet, debouncedSearch, t.loadError]);
        ...
        <>
          ...
          <section className="searchCard networkCard">
            <div className="searchField">
              ...
              />
              {search ? (
                <button type="button" className="clearSearch" onClick={() => setSearch('')} aria-label="Clear search">×</button>
              ) : null}
            </div>
            {search.length > 0 && search.length < 3 ? (
              <small className="searchHint">{t.searchHint}</small>
            ) : null}
            {debouncedSearch.length >= 3 ? (
              <div className="searchResults">
                <strong>{t.searchResults}</strong>
                {data.searchResults.length > 0 ? data.searchResults.map((result) => (
                  <button type="button" key={result.wallet} onClick={() => focusMember(result.wallet)}>
                    <span>{shortWallet(result.wallet, true)}</span>
                    <small>{t.generation} {result.depth}</small>
                    <span aria-hidden="true">›</span>
                  </button>
                )) : loading ? (
                  <div className="searchLoading" aria-hidden="true" />
                ) : (
          ...
          <section ref={directListRef} className="directSection">
            <div className="sectionHeading">
              <div>
                <span className="eyebrow">{t.directNetwork}</span>
                <h2>{data.summary.direct.toLocaleString()} {t.direct}</h2>
              </div>
