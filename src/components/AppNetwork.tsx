  ...
  return (
    ...
    >
                        ...
                        <div className="groupsList">
                          {activeWorkspace.groups.map((group) => (
                            <button type="button" key={group.id} onClick={() => setManagedGroupId(group.id)}>
                              ...
                              <small>{group.members.length} {w.members} {profileDirection === 'rtl' ? '‹' : '›'}</small>
                            </button>
                          ))}
                        </div>
                      ) : <p>{w.noGroups}</p>}
      ...
      <style jsx>{`
        ...
        .compactControls{flex:0 0 auto;margin-left:auto;display:flex;align-items:center;gap:3px}.networkStage{position:relative;flex:1 1 auto;min-height:0;height:auto;overflow:hidden;touch-action:none;overscroll-behavior:contain;background:radial-gradient(ellipse at 50% 50%,rgba(244,183,40,.036),transparent 36%),#080807;cursor:grab;user-select:none;-webkit-user-select:none;-webkit-touch-callout:none}.networkStage:active{cursor:grabbing}.networkStage.layoutEditing{box-shadow:inset 0 0 0 1px rgba(244,183,40,.11)}
        ...
        .inlineError{position:absolute;z-index:90;left:50%;bottom:54px;transform:translateX(-50%);max-width:calc(100% - 28px);padding:8px 9px 8px 11px;display:flex;align-items:center;gap:8px;border:1px solid rgba(194,118,90,.2);border-radius:10px;background:rgba(38,23,18,.96);color:#c7a294;font-size:.53rem;box-shadow:0 12px 30px rgba(0,0,0,.32)}.inlineError span{min-width:0;overflow-wrap:anywhere}.inlineError button{border:0;background:transparent;color:#9f7d71;font-size:.8rem;cursor:pointer}
