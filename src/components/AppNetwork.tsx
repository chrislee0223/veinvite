...
export function AppNetwork({ locale }: { locale: Locale }) {
  ...
  }, [currentData, stageSize, visibleChildren, visibleGroups, positionedInviteSlots]);

  const sparseRootIntro = Boolean(
    rootEntryFitView &&
  ...
  return (
    <section
      className={`networkCard networkCanvasPage${introActive ? ' introActive' : ''}${sparseRootIntro ? ' sparseRootIntro' : ''}`}
    ...
    >
        ...
        <div className="compactControls">
          ...
          <div className="layoutControls">
            ...
            <div className="groupMenuAnchor">
              ...
              {groupDraft ? (
                <aside className="groupBuilder" data-no-pan="true" dir={profileDirection}>
                  ...
                  >
                    ...
                    <small aria-live="polite">{u.peopleCount.replace('{count}', String(groupDraft.members.length))}</small>
                  </div>
                ...
                <aside className="groupsPanel" data-no-pan="true" dir={profileDirection}>
                  {managedGroup ? (
                    <>
                      ...
                      <small className="groupManageCount">{u.savedCount.replace('{count}', String(managedGroup.members.length))}</small>
                      <div className="groupManageMembers">
                        {managedGroup.members.map((member) => (
                          <div className="groupManageMember" key={member} title={member}>
                            <span dir="ltr">{shortWallet(member)}</span>
                            <button
                              type="button"
                        ...
                        <div className="groupsList">
                          {activeWorkspace.groups.map((group) => (
                            <button type="button" key={group.id} onClick={() => setManagedGroupId(group.id)}>
                              <span dir="auto">{group.label || w.group}</span>
                              <small>{u.peopleCount.replace('{count}', String(group.members.length))} {profileDirection === 'rtl' ? '‹' : '›'}</small>
                            </button>
                          ))}
                        </div>
                      ) : <p>{w.noGroups}</p>}
      ...
      {searchOpen ? (
        <div className="networkSearchRow" data-no-pan="true">
          <div className="searchWrap">
            ...
            {!editingLayout && searchQuery.trim().length >= 3 ? (
              <div className="searchResults" role="listbox" dir={profileDirection}>
                {searching ? (
      ...
      >
        ...
        >
          <div className={`worldContent ${rootSceneReady ? 'sceneReady' : 'scenePending'}${navigationDirection ? ` nav-${navigationDirection}` : ''}`}>
            ...
            >
              ...
              <span className={`nodeMeta${focusIsRoot ? ' rootIdentityOnly' : ''}`}>
            ...
            {visibleGroups.map((group) => {
              ...
              return (
                <button
                  ...
                  data-workspace-draggable={editingLayout ? 'true' : undefined}
                >
                  <span className="groupGlyph" aria-hidden="true"><GroupsControlGlyph size={16} /></span>
                  <strong dir="auto">{group.label || w.group}</strong>
                  <small>{u.peopleCount.replace('{count}', String(group.members.length))}</small>
                </button>
              );
            })}
            ...
            {positionedInviteSlots.map((slot) => {
              ...
              const progressStyle = {
                left: slot.x,
                top: slot.y,
                ...(pendingAcceptance ? {} : { '--slot-progress': progressDegrees }),
              } as CSSProperties;
              const slotStatusLabel = pendingAcceptance ? t.pendingAcceptance : t.inProgress;

              return (
                <button
                  type="button"
                  className={`personNode childNode ${pendingAcceptance ? 'pendingInviteNode' : 'progressInviteNode'}${editingLayout ? ' draggable' : ''}${isDragging ? ' dragging' : ''}`}
                  key={slot.key}
                  style={progressStyle}
                  onPointerDown={handlePointerDown}
                  onContextMenu={(event) => event.preventDefault()}
                  ...
                  title={slot.inviteeWallet ?? slotStatusLabel}
                >
        ...
        {!focusIsRoot ? (
          ...
          >
            {profileDirection === 'rtl' ? '›' : '‹'} {t.invitedBy}
          </button>
        ) : null}
      ...
      <style jsx>{`
        ...
        .networkSearchRow{position:absolute;z-index:85;top:44px;inset-inline:6px;padding:5px;box-sizing:border-box;border:1px solid rgba(255,205,80,.13);border-radius:12px;background:rgba(11,11,9,.97);box-shadow:0 12px 30px rgba(0,0,0,.38);backdrop-filter:blur(10px)}.searchWrap{position:relative;width:100%}.searchField{width:100%;height:32px;box-sizing:border-box;padding-block:0;padding-inline:8px 3px;display:flex;align-items:center;gap:5px;border:1px solid rgba(255,205,80,.14);border-radius:9px;background:#11110f;color:#8f887d}.searchField:focus-within{border-color:rgba(244,183,40,.34)}.searchField input{min-width:0;width:100%;height:30px;box-sizing:border-box;padding:0;border:0;background:transparent;color:#d8d3ca;font:inherit;font-size:16px;line-height:1.2;outline:none;user-select:text;-webkit-user-select:text;-webkit-touch-callout:default}.searchField input:disabled{opacity:.45}.searchClose{flex:0 0 25px;width:25px;height:25px;padding:0;border:0;border-radius:7px;background:transparent;color:#716d66;font:inherit;font-size:.9rem;cursor:pointer}.searchClose:hover{background:rgba(255,255,255,.035);color:#b4ada3}.searchResults{position:absolute;z-index:90;top:37px;inset-inline-start:0;width:100%;max-height:245px;overflow:auto;padding:5px;border:1px solid rgba(255,205,80,.14);border-radius:12px;background:rgba(14,14,12,.985);box-shadow:0 18px 40px rgba(0,0,0,.42)}.searchResults button{width:100%;padding:8px;border:0;border-radius:8px;background:transparent;color:#ddd7cc;text-align:start;cursor:pointer}.searchResults button:hover{background:rgba(244,183,40,.06)}.searchResults strong{display:block;font-size:.62rem}.searchResults button span{display:block;margin-top:3px;color:#6f6b64;font-size:.52rem}.searchStatus{display:block;padding:11px 8px;color:#77736c;font-size:.56rem;line-height:1.45;text-align:center}
        .compactControls{flex:0 0 auto;margin-inline-start:auto;display:flex;align-items:center;gap:3px}.networkStage{position:relative;flex:1 1 auto;min-height:0;height:auto;overflow:hidden;touch-action:none;overscroll-behavior:contain;background:radial-gradient(ellipse at 50% 50%,rgba(244,183,40,.036),transparent 36%),#080807;cursor:grab;user-select:none;-webkit-user-select:none;-webkit-touch-callout:none}.networkStage:active{cursor:grabbing}.networkStage.layoutEditing{box-shadow:inset 0 0 0 1px rgba(244,183,40,.11)}
        .world{position:absolute;top:0;left:0;will-change:transform;backface-visibility:hidden}.world.cameraTransition{transition:transform ${NAVIGATION_MS}ms cubic-bezier(.18,.82,.2,1)}.introActive .world.cameraTransition{transition-duration:${FIT_TRANSITION_MS}ms}.worldContent{position:absolute;inset:0;transform-origin:${FOCUS_X}px ${FOCUS_Y}px}.worldContent.scenePending{opacity:0;pointer-events:none}.worldContent.sceneReady{opacity:1;transition:opacity 120ms ease-out}.sparseRootIntro.introActive .worldContent.sceneReady{animation:networkSparseRootIntro ${FIT_TRANSITION_MS}ms ${INTRO_HOLD_MS}ms cubic-bezier(.18,.82,.2,1) both}.worldContent.nav-forward{animation:networkForward ${NAVIGATION_MS}ms cubic-bezier(.18,.82,.2,1)}.worldContent.nav-back{animation:networkBack ${NAVIGATION_MS}ms cubic-bezier(.18,.82,.2,1)}
        ...
        .personNode,.slotNode,.groupNode{position:absolute;z-index:6;transform:translate(-50%,-50%);font:inherit;translate:none}.personNode{min-width:0;width:52px;height:52px;padding:0;border:0;border-radius:50%;background:transparent;color:#d9d4ca;display:block;cursor:pointer;user-select:none;-webkit-user-select:none;-webkit-touch-callout:none;touch-action:none;isolation:isolate}.focusNode{min-width:0;width:74px;height:74px;z-index:8}.childNode::before,.slotNode::before{content:'';position:absolute;left:50%;top:50%;border-radius:50%;transform:translate(-50%,-50%);pointer-events:none;z-index:0}.childNode::before{width:58px;height:58px;background:radial-gradient(circle,rgba(8,8,7,.94) 0 87%,rgba(8,8,7,.58) 91%,rgba(8,8,7,.17) 96%,rgba(8,8,7,0) 100%)}.slotNode::before{width:58px;height:58px;background:radial-gradient(circle,rgba(8,8,7,.92) 0 86%,rgba(8,8,7,.54) 91%,rgba(8,8,7,.15) 96%,rgba(8,8,7,0) 100%)}.nodeCircle,.slotCircle{position:absolute;inset:0;z-index:1;display:grid;place-items:center;border-radius:50%;box-sizing:border-box;background:#0d0d0b;overflow:hidden;transition:transform 170ms ease,border-color 170ms ease,box-shadow 170ms ease}.nodeCircle{border:1px solid rgba(210,174,65,.38);box-shadow:0 0 22px rgba(244,183,40,.025);transform:scale(var(--network-node-scale,1))}.focusCircle{inset:7px;z-index:2;border-color:rgba(255,207,71,.82);background:radial-gradient(circle at 50% 45%,rgb(24,21,13) 0%,rgb(13,13,11) 62%,rgb(13,13,11) 100%);box-shadow:0 0 0 1px rgba(244,183,40,.07),0 0 28px rgba(244,183,40,.08);transform:scale(var(--network-center-scale,1))}.focusNode::after{content:'';position:absolute;inset:0;z-index:0;border-radius:50%;background:radial-gradient(circle at 50% 45%,rgb(24,21,13) 0%,rgb(13,13,11) 62%,rgb(13,13,11) 100%);scale:var(--network-center-scale,1);transition:scale 170ms ease;pointer-events:none}.focusNode::before{content:'';position:absolute;inset:0;z-index:1;border:1px solid rgba(244,183,40,.42);border-radius:50%;box-shadow:0 0 18px rgba(244,183,40,.055);scale:var(--network-center-scale,1);transition:scale 170ms ease;animation:networkYouBreath 2.8s ease-in-out infinite;pointer-events:none}.introActive .focusNode::before{animation:networkYouIntro .72s ease-out both}.sparseRootIntro.introActive .focusNode::before{animation:none;opacity:.46;transform:scale(.96)}.personNode:hover .nodeCircle,.personNode:focus-visible .nodeCircle,.personNode.selected .nodeCircle{border-color:rgba(244,183,40,.78);box-shadow:0 0 0 3px rgba(244,183,40,.08),0 0 26px rgba(244,183,40,.1);transform:scale(var(--network-node-selected-scale,1.07))}.personNode.selected .nodeCircle{border-color:rgba(255,211,88,.96);box-shadow:0 0 0 4px rgba(244,183,40,.11),0 0 34px rgba(244,183,40,.18)}.personNode.selected .nodeMeta{opacity:1}.focusNode:hover .focusCircle,.focusNode:focus-visible .focusCircle,.focusNode.selected .focusCircle{transform:scale(var(--network-center-selected-scale,1.07))}.childNode.status-rewarded .nodeCircle{border-color:rgba(232,183,62,.58)}.childNode.status-qualified .nodeCircle{border-color:rgba(193,166,90,.46)}.personNode.draggable,.slotNode.draggable,.groupNode.draggable{cursor:grab}.personNode.draggable:active,.slotNode.draggable:active,.groupNode.draggable:active{cursor:grabbing}.personNode.dragging,.slotNode.dragging{z-index:14}.personNode.dragGhostSource{opacity:.12}.personNode.dragging .nodeCircle,.slotNode.dragging .slotCircle{border-color:rgba(244,183,40,.92);box-shadow:0 0 0 4px rgba(244,183,40,.12),0 0 30px rgba(244,183,40,.18)}.groupNode.dragging{z-index:14;border-color:rgba(244,183,40,.82);box-shadow:0 14px 30px rgba(0,0,0,.34),0 0 0 3px rgba(244,183,40,.1)}.personNode.grouping{animation:groupDropAway ${GROUP_DROP_MS}ms ease forwards}.personNode.restoring{animation:groupNodeRestore ${GROUP_DROP_MS}ms cubic-bezier(.2,.8,.2,1) both}
        .nodeCircle :global(.identity){width:100%;height:100%;display:grid;place-items:center}.nodeCircle :global(.avatarSlot){position:relative;display:grid;place-items:center}.childNode .nodeCircle :global(.avatarSlot),.childNode .nodeCircle :global(.neutralAvatar),.childNode .nodeCircle :global(.avatarSlot img){width:40px!important;height:40px!important}.focusNode .nodeCircle :global(.avatarSlot),.focusNode .nodeCircle :global(.neutralAvatar),.focusNode .nodeCircle :global(.avatarSlot img){width:56px!important;height:56px!important}.nodeCircle :global(.neutralAvatar){display:grid;place-items:center;border:0;border-radius:50%;background:#171611;color:#8e7b50}.nodeCircle :global(.avatarSlot img){position:absolute;inset:0;margin:auto;border-radius:50%;object-fit:cover;transition:opacity 160ms ease}.nodeMeta{position:absolute;left:50%;z-index:2;width:92px;display:grid;justify-items:center;gap:1px;transform:translateX(-50%);opacity:var(--network-label-opacity,1);pointer-events:none;transition:opacity 90ms linear}.childNode .nodeMeta{top:calc(100% + 6px)}.focusNode .nodeMeta{top:calc(100% + 7px);width:110px}.focusNode .nodeMeta.rootIdentityOnly{top:calc(100% + 8px)}.nodeMeta strong{max-width:90px;color:#89837a;font-size:.46rem;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.nodeNetworkMetric{display:inline-flex;align-items:center;justify-content:center;gap:3px;color:#d8b958;font-size:.48rem;font-weight:900;font-variant-numeric:tabular-nums;white-space:nowrap}.nodeNetworkMetric svg{flex:0 0 auto}.nodeProgressStatus{max-width:90px;overflow:hidden;text-overflow:ellipsis;color:#b28c32!important;font-size:.43rem!important;font-weight:900;white-space:nowrap}.focusNode .nodeMeta strong{color:#8b857c;font-size:.48rem}.focusNode .nodeNetworkMetric{color:#edc65c;font-size:.52rem}.nodeBusy{position:absolute;z-index:3;inset-inline-end:-2px;top:-2px;width:7px;height:7px;border-radius:50%;background:#e9bc45;box-shadow:0 0 10px rgba(233,188,69,.8);animation:pulse 900ms ease-in-out infinite alternate}
        .groupNode{width:60px;height:60px;min-width:60px;max-width:60px;padding:4px 3px;box-sizing:border-box;border:1.2px solid rgba(224,183,74,.62);border-radius:13px;background:linear-gradient(180deg,rgba(25,22,14,.97),rgba(16,15,11,.97));color:#d8b450;display:grid;grid-template-columns:1fr;grid-template-rows:18px auto auto;row-gap:2px;align-content:center;justify-items:center;text-align:center;box-shadow:0 5px 17px rgba(0,0,0,.24);cursor:pointer;touch-action:none;user-select:none;-webkit-user-select:none;-webkit-touch-callout:none}.groupNode:hover,.groupNode.dropTarget{border-color:rgba(255,207,71,.84);box-shadow:0 0 0 3px rgba(244,183,40,.1),0 8px 26px rgba(0,0,0,.28)}.groupNode.dropTarget{background:linear-gradient(180deg,rgba(35,29,14,.99),rgba(20,17,10,.98));transform:translate(-50%,-50%) scale(1.035)}.groupNode.expanded{border-color:rgba(235,193,84,.72);background:linear-gradient(180deg,rgba(27,24,15,.98),rgba(17,16,12,.96))}.groupNode.expanded.dropTarget{border-color:rgba(255,207,71,.9);background:linear-gradient(180deg,rgba(38,31,14,.99),rgba(22,18,10,.98));box-shadow:0 0 0 3px rgba(244,183,40,.11),0 8px 26px rgba(0,0,0,.28)}.groupNode.created{animation:groupHubIn ${GROUP_HUB_IN_MS}ms cubic-bezier(.16,.82,.2,1) both}.groupNode .groupGlyph{grid-row:1}.groupNode strong{min-width:0;max-width:52px;font-size:.44rem;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.groupNode small{min-width:0;max-width:52px;color:#857655;font-size:.31rem;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.groupGlyph{width:18px;height:18px;display:grid;place-items:center;color:#d6ad49}.groupGlyph svg{display:block}
        .slotNode{width:52px;height:52px;padding:0;border:0;border-radius:50%;background:transparent;color:#c79f36;cursor:pointer;touch-action:none;user-select:none;-webkit-user-select:none;isolation:isolate}.slotCircle{border:1px dashed rgba(226,181,62,.52);color:#c79f36;font-size:.9rem;background:#0d0d0b;transform:scale(var(--network-node-scale,1));animation:slotPulse 5.6s ease-in-out infinite}.slotNode>strong{position:absolute;left:50%;top:calc(100% + 5px);z-index:2;width:92px;transform:translateX(-50%);overflow:hidden;text-overflow:ellipsis;color:#a98735;font-size:.47rem;white-space:nowrap;pointer-events:none;opacity:var(--network-label-opacity,1)}.slotNode:hover .slotCircle,.slotNode:focus-visible .slotCircle{border-style:solid;border-color:rgba(244,183,40,.9);box-shadow:0 0 28px rgba(244,183,40,.1);transform:scale(var(--network-node-selected-scale,1.07))}.pendingInviteNode .nodeCircle{border-style:dashed;border-color:rgba(226,181,62,.52);background:#0d0d0b}.progressInviteNode::after{content:'';position:absolute;z-index:0;inset:-3px;border-radius:50%;background:conic-gradient(rgba(244,183,40,.95) var(--slot-progress),rgba(244,183,40,.12) 0);-webkit-mask:radial-gradient(farthest-side,transparent calc(100% - 3px),#000 0);mask:radial-gradient(farthest-side,transparent calc(100% - 3px),#000 0);filter:drop-shadow(0 0 4px rgba(244,183,40,.18));pointer-events:none}.progressInviteNode .nodeCircle{border-color:rgba(244,183,40,.62);background:#11100c}.progressInviteNode .nodeMeta small{color:#b28c32}.pendingInviteGlyph{display:grid;place-items:center;width:100%;height:100%;color:#d3aa43;font-size:.9rem;font-weight:950;letter-spacing:.08em}
        ...
        .inlineError{position:absolute;z-index:90;left:50%;bottom:54px;transform:translateX(-50%);max-width:calc(100% - 28px);padding-block:8px;padding-inline:11px 9px;display:flex;align-items:center;gap:8px;border:1px solid rgba(194,118,90,.2);border-radius:10px;background:rgba(38,23,18,.96);color:#c7a294;font-size:.53rem;box-shadow:0 12px 30px rgba(0,0,0,.32)}.inlineError span{min-width:0;overflow-wrap:anywhere}.inlineError button{border:0;background:transparent;color:#9f7d71;font-size:.8rem;cursor:pointer}
        @keyframes networkSparseRootIntro{0%{scale:1.14}100%{scale:1}}@keyframes networkForward{0%{opacity:.68;scale:.975}100%{opacity:1;scale:1}}@keyframes networkBack{0%{opacity:.74;scale:1.035}100%{opacity:1;scale:1}}@keyframes networkSlotFlow{from{stroke-dashoffset:43}to{stroke-dashoffset:-43}}@keyframes networkYouBreath{0%,100%{opacity:.46;transform:scale(.96)}50%{opacity:.92;transform:scale(1.06)}}@keyframes networkYouIntro{0%{opacity:.25;transform:scale(.78)}58%{opacity:1;transform:scale(1.14)}100%{opacity:.46;transform:scale(.96)}}@keyframes networkNodeBloom{0%{transform:scale(.45);box-shadow:0 0 0 rgba(244,183,40,0)}55%{transform:scale(1.18);box-shadow:0 0 42px rgba(244,183,40,.22)}100%{transform:scale(var(--network-node-scale,1));box-shadow:0 0 22px rgba(244,183,40,.025)}}@keyframes slotPulse{0%,100%{box-shadow:0 0 0 rgba(244,183,40,0)}50%{box-shadow:0 0 22px rgba(244,183,40,.07)}}@keyframes pulse{to{opacity:.38;transform:scale(.82)}}@keyframes groupDropAway{to{opacity:0;scale:.82}}@keyframes groupNodeRestore{0%{opacity:0;scale:.86}100%{opacity:1;scale:1}}@keyframes groupHubIn{0%{opacity:0;scale:.82}65%{opacity:1;scale:1.04}100%{opacity:1;scale:1}}@keyframes groupMemberIn{0%{opacity:0;translate:0 5px}100%{opacity:1;translate:0 0}}@keyframes groupCountPop{0%{scale:.8;opacity:.45}60%{scale:1.12;opacity:1}100%{scale:1;opacity:1}}
        @media(prefers-reduced-motion:reduce){.world.cameraTransition{transition:none}.worldContent.sceneReady{transition:none}.sparseRootIntro.introActive .worldContent.sceneReady{animation:none!important}.groupDropZone,.createFirstGroup{transition:none}.worldContent.nav-forward,.worldContent.nav-back,.slotEdgePulse,.nodeBusy,.personNode.grouping,.personNode.restoring,.groupNode.created,.groupDraftMembers button,.groupDropZone.memberAdded small,.slotCircle,.focusNode::before,.worldContent.nav-forward .childNode .nodeCircle{animation:none!important}}
      `}</style>
    </section>
  );
}
