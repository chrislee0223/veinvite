...
const AppNetworkCanaryV50 = dynamic(
  () => import('./AppNetworkCanaryV50').then((module) => module.AppNetworkCanaryV50),
  { ssr: false },
);
...
export function AppGuide({ locale }: { locale: Locale }) {
  ...
  if (wallet?.toLowerCase() === NETWORK_CANARY_WALLET) {
    return <AppNetworkCanaryV50 key={wallet.toLowerCase()} locale={locale} />;
  }
