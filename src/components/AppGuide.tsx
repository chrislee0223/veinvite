...
const AppNetworkCanaryV65 = dynamic(
  () => import('./AppNetworkCanaryV65').then((module) => module.AppNetworkCanaryV65),
  { ssr: false },
);
...
export function AppGuide({ locale }: { locale: Locale }) {
  ...
  if (wallet?.toLowerCase() === NETWORK_CANARY_WALLET) {
    return <AppNetworkCanaryV65 key={wallet.toLowerCase()} locale={locale} />;
  }
