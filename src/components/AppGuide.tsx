...
const AppNetworkCanaryV63 = dynamic(
  () => import('./AppNetworkCanaryV63').then((module) => module.AppNetworkCanaryV63),
  { ssr: false },
);
...
export function AppGuide({ locale }: { locale: Locale }) {
  ...
  if (wallet?.toLowerCase() === NETWORK_CANARY_WALLET) {
    return <AppNetworkCanaryV63 key={wallet.toLowerCase()} locale={locale} />;
  }
