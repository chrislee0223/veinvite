...
const AppNetworkCanaryV60 = dynamic(
  () => import('./AppNetworkCanaryV60').then((module) => module.AppNetworkCanaryV60),
  { ssr: false },
);
...
export function AppGuide({ locale }: { locale: Locale }) {
  ...
  if (wallet?.toLowerCase() === NETWORK_CANARY_WALLET) {
    return <AppNetworkCanaryV60 key={wallet.toLowerCase()} locale={locale} />;
  }
