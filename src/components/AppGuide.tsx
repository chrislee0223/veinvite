...
const AppNetworkCanaryV57 = dynamic(
  () => import('./AppNetworkCanaryV57').then((module) => module.AppNetworkCanaryV57),
  { ssr: false },
);
...
export function AppGuide({ locale }: { locale: Locale }) {
  ...
  if (wallet?.toLowerCase() === NETWORK_CANARY_WALLET) {
    return <AppNetworkCanaryV57 key={wallet.toLowerCase()} locale={locale} />;
  }
