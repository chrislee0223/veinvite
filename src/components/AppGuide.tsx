...
const AppNetworkCanaryV67 = dynamic(
  () => import('./AppNetworkCanaryV67').then((module) => module.AppNetworkCanaryV67),
  { ssr: false },
);
...
export function AppGuide({ locale }: { locale: Locale }) {
  ...
  if (wallet?.toLowerCase() === NETWORK_CANARY_WALLET) {
    return <AppNetworkCanaryV67 key={wallet.toLowerCase()} locale={locale} />;
  }
