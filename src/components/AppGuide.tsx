...
const AppNetworkCanaryV56 = dynamic(
  () => import('./AppNetworkCanaryV56').then((module) => module.AppNetworkCanaryV56),
  { ssr: false },
);
...
export function AppGuide({ locale }: { locale: Locale }) {
  ...
  if (wallet?.toLowerCase() === NETWORK_CANARY_WALLET) {
    return <AppNetworkCanaryV56 key={wallet.toLowerCase()} locale={locale} />;
  }
