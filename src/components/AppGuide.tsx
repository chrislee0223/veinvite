...
const AppNetworkCanaryV51 = dynamic(
  () => import('./AppNetworkCanaryV51').then((module) => module.AppNetworkCanaryV51),
  { ssr: false },
);
...
export function AppGuide({ locale }: { locale: Locale }) {
  ...
  if (wallet?.toLowerCase() === NETWORK_CANARY_WALLET) {
    return <AppNetworkCanaryV51 key={wallet.toLowerCase()} locale={locale} />;
  }
