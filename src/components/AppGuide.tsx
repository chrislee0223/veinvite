...
const AppNetworkCanaryV62 = dynamic(
  () => import('./AppNetworkCanaryV62').then((module) => module.AppNetworkCanaryV62),
  { ssr: false },
);
...
export function AppGuide({ locale }: { locale: Locale }) {
  ...
  if (wallet?.toLowerCase() === NETWORK_CANARY_WALLET) {
    return <AppNetworkCanaryV62 key={wallet.toLowerCase()} locale={locale} />;
  }
