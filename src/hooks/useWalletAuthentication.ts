        ...
        run = (async () => {
          ...
          try {
            ...
            if (
              ...
              // VeWorld can briefly publish the account before DAppKit
              // republishes its source after a wallet switch. Falling back to
              // certificate during that gap can resurrect the old native
              // Sign-certificate spinner. Wait for source ownership instead.
              if (!settledDappKitSource) {
                throw new Error(
                  'Wallet connection is still synchronizing. Please try again.',
                );
