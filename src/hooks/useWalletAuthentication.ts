        ...
        run = (async () => {
          ...
          try {
            ...
            if (
              ...
              // VeWorld can briefly unpublish the DAppKit source during an
              // account switch even though the signer/account are already
              // stable. Never choose the legacy certificate path solely from
              // that transient gap; it can produce a first-attempt proof that
              // the server rejects before typed-data succeeds on retry.
              if (!settledDappKitSource) {
                throw new Error(
                  'Wallet connection is still synchronizing. Please try again.',
                );
