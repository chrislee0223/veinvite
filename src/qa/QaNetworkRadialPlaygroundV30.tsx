...
function basePersonPoint(id: string, localIndex: number, compact: boolean) {
  ...
  const jitterY = (((hash >>> 6) % 17) - 8) * (compact ? .65 : 1.2);
...
export function QaNetworkRadialPlaygroundV30() {
  ...
  return (
    <main className="page">
      <section className="labHeader">
        <div>
          <strong>RADIAL NETWORK PLAYGROUND · V30</strong>
          <span>Long-press to edit · stable anchors · normalized custom positions · live-following lines</span>
        </div>
        ...
        <div ref={stageRef} className={`stage ${editMode ? 'editMode' : ''} phase-${phase} ${pageMotion ? `page-${pageMotion}` : ''}`} onWheel={onWheelZoom}>
          ...
          <div className={`scene ${zoomClass}`} style={{ '--sceneScale': zoom } as CSSProperties}>
            ...
            <svg className="edges" viewBox="-360 -285 720 620" aria-hidden="true">
              <defs>
                <linearGradient id="base30" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="rgba(231,204,126,.145)"/><stop offset="1" stopColor="rgba(231,204,126,.04)"/></linearGradient>
                <filter id="blur30" x="-100%" y="-100%" width="300%" height="300%"><feGaussianBlur stdDeviation="5"/></filter>
                {slotItems.map((slot, index) => <radialGradient key={`gradient-${slot.id}`} id={`gateGlow30-${index}`} gradientUnits="userSpaceOnUse" cx="0" cy="0" r={compact ? 50 : 70}><stop offset="0" stopColor="rgba(255,207,78,.84)"/><stop offset=".38" stopColor="rgba(244,183,40,.27)"/><stop offset="1" stopColor="rgba(244,183,40,0)"/><animate attributeName="cx" values={`0;${slot.x * .5};${slot.x}`} dur="8s" begin={`${index * 4}s`} repeatCount="indefinite"/><animate attributeName="cy" values={`0;${slot.y * .5};${slot.y}`} dur="8s" begin={`${index * 4}s`} repeatCount="indefinite"/></radialGradient>)}
              </defs>
              {ringItems.map((item) => {
                const key = layoutKey(item.kind, item.id);
          ...
          {editMode ? <div className="editHint">Drag people or Available · overlaps are allowed · lines follow automatically</div> : <div className="zoomHint">hold a node to edit · wheel / − + · {Math.round(zoom * 100)}%</div>}
      ...
      <section className="rules">
        <span><b>Hold to edit</b>노드를 약 0.5초 누르면 편집 모드로 들어가고 그대로 끌어서 이동</span>
        <span><b>Gesture safe</b>누르는 동안 10px 이상 움직이면 길게 누르기를 취소해 스크롤 오작동 방지</span>
        <span><b>Stable + personal</b>기본 위치는 고정 · 직접 옮긴 위치는 상대 좌표로 저장</span>
      ...
      <style jsx>{`
        ...
        .edges{position:absolute;left:50%;top:50%;width:720px;height:620px;transform:translate(-50%,-50%);overflow:visible;pointer-events:none;z-index:2;transition:opacity 180ms ease}.spoke{fill:none;stroke:url(#base30);stroke-width:1;stroke-linecap:round;transition:opacity 120ms ease,stroke 120ms ease}.slotSpoke{stroke:rgba(213,177,73,.18)}.spoke.dragActive{stroke:rgba(242,196,68,.58);stroke-width:1.35}.spoke.dragDim,.gateGlowPath.dragDim{opacity:.16!important}.gateGlowPath{fill:none;stroke-width:8;stroke-linecap:round;opacity:0}
        .centerWrap{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);z-index:8}.centerFloat{width:132px;display:grid;justify-items:center;gap:5px;animation:centerFloat30 7.6s ease-in-out infinite}.centerCircle{width:74px;height:74px;border-radius:50%;display:grid;place-items:center;background:#0d0d0b;border:1px solid rgba(244,183,40,.66);color:#e5b943;font-size:.72rem;box-shadow:0 0 38px rgba(244,183,40,.045)}.centerFloat b{font-size:.62rem}.centerFloat small{font-size:.42rem;color:#756e63;white-space:nowrap}
        .ringLayer{position:absolute;left:50%;top:50%;z-index:6;transition:opacity 180ms ease}.ringNode{--x:0px;--y:0px;position:absolute;left:0;top:0;width:110px;transform:translate(calc(var(--x) - 50%),calc(var(--y) - 50%));display:grid;justify-items:center;color:#d8d2c5;border:0;background:transparent;animation:bloom30 430ms cubic-bezier(.18,.8,.24,1) both;animation-delay:var(--delay,0ms);user-select:none;-webkit-user-select:none;-webkit-touch-callout:none;touch-action:manipulation}.floatInner{display:grid;justify-items:center;gap:4px;animation:float30 var(--floatDur,6.4s) ease-in-out infinite;animation-delay:var(--floatDelay,-1s);pointer-events:none}.ringCircle,.slotCircle,.miniCircle,.travelCircle{border-radius:50%;display:grid;place-items:center;background:#0d0d0b}.ringCircle{width:52px;height:52px;border:1px solid rgba(210,174,65,.37);color:#d9b34a;font-size:.62rem;transition:border-color 160ms ease,box-shadow 160ms ease,transform 160ms ease}.ringNode b{font-size:.48rem;white-space:nowrap}.ringNode small{font-size:.39rem;color:#6d665c;white-space:nowrap}.person:hover .ringCircle,.person:focus-visible .ringCircle{border-color:rgba(244,183,40,.72);box-shadow:0 0 24px rgba(244,183,40,.08);transform:scale(1.04)}
        .ringNode.pressing .floatInner{animation:none}.ringNode.pressing .ringCircle,.ringNode.pressing .slotCircle{animation:hold30 ${LONG_PRESS_MS}ms cubic-bezier(.2,.75,.2,1) forwards}.editMode .ringNode{cursor:grab;touch-action:none}.editMode .ringNode.dragging{cursor:grabbing;z-index:22}.editMode .ringNode.dragging .floatInner{animation:none}.editMode .ringNode.dragging .ringCircle,.editMode .ringNode.dragging .slotCircle{border-color:rgba(244,183,40,.9);box-shadow:0 0 26px rgba(244,183,40,.14);transform:scale(1.06)}
        .slot{opacity:.82}.slotCircle{width:46px;height:46px;border:1px dashed rgba(226,181,62,.5);color:#c79f36;font-size:.9rem;animation:gatePulse30 8s ease-in-out infinite;animation-delay:var(--gateDelay,0s)}.slot b{color:#a3936e}.slot small{color:#675f53}.slot.joining .slotCircle{border-style:solid;animation:joining30 .9s ease-in-out infinite}.slot.complete .slotCircle{border-style:solid;border-color:rgba(138,205,123,.68);color:#8fca80;box-shadow:0 0 22px rgba(115,189,101,.1)}.newArrival .ringCircle{animation:newArrival30 650ms cubic-bezier(.16,.8,.2,1) both}
        ...
        .traveler{--tx:0px;--ty:0px;position:absolute;left:50%;top:50%;width:110px;display:grid;justify-items:center;gap:4px;z-index:30;color:#e8dfcb;pointer-events:none}.travelCircle{width:56px;height:56px;border:1px solid rgba(244,183,40,.72);color:#e5b943;box-shadow:0 0 34px rgba(244,183,40,.09)}.traveler b{font-size:.49rem}.traveler.in{animation:travelIn30 500ms cubic-bezier(.18,.78,.18,1) both}.traveler.out{animation:travelOut30 500ms cubic-bezier(.18,.78,.18,1) both}
        .phase-depart .ringLayer,.phase-depart .edges{opacity:.12}.phase-depart .centerWrap{animation:centerYield30 500ms cubic-bezier(.18,.78,.18,1) both}.phase-arrive .centerWrap{animation:centerArrive30 330ms cubic-bezier(.18,.78,.18,1) both}.phase-arrive .ringNode{animation:bloom30 430ms cubic-bezier(.18,.8,.24,1) both;animation-delay:var(--delay,0ms)}.page-left .ringLayer,.page-right .ringLayer{animation:pageFade30 300ms ease both}
        ...
        .zoomHint,.editHint{position:absolute;right:10px;bottom:10px;padding:5px 7px;border:1px solid rgba(255,255,255,.04);border-radius:8px;background:rgba(12,12,10,.66);font-size:.38rem;color:#5f594f;z-index:42;pointer-events:none}.editHint{color:#a08b58;border-color:rgba(244,183,40,.11)}.arrivalNotice,.editNotice{position:absolute;left:50%;bottom:38px;transform:translateX(-50%);padding:7px 10px;border-radius:10px;background:rgba(10,16,10,.92);font-size:.43rem;z-index:44;animation:notice30 1600ms ease both;white-space:nowrap}.arrivalNotice{border:1px solid rgba(138,205,123,.2);color:#91c886}.editNotice{border:1px solid rgba(244,183,40,.22);color:#d5b65e}.debugPanel{position:absolute;left:10px;bottom:10px;display:flex;flex-wrap:wrap;gap:5px;max-width:70%;z-index:45}.debugPanel span{padding:5px 7px;border:1px solid rgba(255,255,255,.05);border-radius:8px;background:rgba(12,12,10,.84);font-size:.4rem;color:#777067}
        ...
        @keyframes float30{0%,100%{transform:translate3d(0,-1px,0) rotate(-.22deg)}35%{transform:translate3d(1.2px,2.4px,0) rotate(.26deg)}70%{transform:translate3d(-1px,.5px,0) rotate(-.1deg)}}
        @keyframes centerFloat30{0%,100%{transform:translateY(0)}45%{transform:translateY(1.7px)}75%{transform:translateY(-1px)}}
        @keyframes bloom30{from{opacity:0;filter:blur(2px);transform:translate(calc(var(--x) - 50%),calc(var(--y) - 46%)) scale(.94)}to{opacity:1;filter:blur(0);transform:translate(calc(var(--x) - 50%),calc(var(--y) - 50%)) scale(1)}}
        @keyframes hold30{0%{transform:scale(1);box-shadow:0 0 0 rgba(244,183,40,0)}65%{transform:scale(1.025);box-shadow:0 0 13px rgba(244,183,40,.08)}100%{transform:scale(1.07);border-color:rgba(244,183,40,.88);box-shadow:0 0 28px rgba(244,183,40,.17)}}
        @keyframes gatePulse30{0%,68%,100%{box-shadow:0 0 0 rgba(244,183,40,0);border-color:rgba(226,181,62,.42)}78%{box-shadow:0 0 22px rgba(244,183,40,.1);border-color:rgba(238,191,64,.68)}}
        @keyframes joining30{0%,100%{transform:scale(1);box-shadow:0 0 10px rgba(244,183,40,.03)}50%{transform:scale(1.06);box-shadow:0 0 24px rgba(244,183,40,.13)}}
        @keyframes newArrival30{0%{opacity:.2;transform:translateY(10px) scale(.72);box-shadow:0 0 0 rgba(244,183,40,0)}60%{opacity:1;transform:translateY(-2px) scale(1.08);box-shadow:0 0 34px rgba(244,183,40,.15)}100%{opacity:1;transform:translateY(0) scale(1);box-shadow:0 0 12px rgba(244,183,40,.04)}}
        @keyframes travelIn30{0%{transform:translate(calc(var(--tx) - 50%),calc(var(--ty) - 50%)) scale(1)}38%{transform:translate(calc(var(--tx)*.62 - 50%),calc(var(--ty)*.62 - 58%)) scale(1.08)}100%{transform:translate(-50%,-50%) scale(1.05)}}
        @keyframes travelOut30{0%{transform:translate(-50%,-50%) scale(1.05)}62%{transform:translate(calc(var(--tx)*.65 - 50%),calc(var(--ty)*.65 - 56%)) scale(1.04)}100%{transform:translate(calc(var(--tx) - 50%),calc(var(--ty) - 50%)) scale(1)}}
        @keyframes centerYield30{to{opacity:.12;transform:translate(-50%,-82%) scale(.72)}}@keyframes centerArrive30{from{opacity:.2;transform:translate(-50%,-43%) scale(.92)}to{opacity:1;transform:translate(-50%,-50%) scale(1)}}@keyframes pageFade30{0%{opacity:.22;filter:blur(1px)}100%{opacity:1;filter:blur(0)}}@keyframes notice30{0%{opacity:0;transform:translate(-50%,7px)}12%,78%{opacity:1;transform:translate(-50%,0)}100%{opacity:0;transform:translate(-50%,-4px)}}
