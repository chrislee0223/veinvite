...
function childEdgePath(from: { x: number; y: number }, to: { x: number; y: number }) {
  const startY = from.y + 40;
  const endY = to.y - 44;
  const mid = startY + (endY - startY) * 0.5;
  return `M${from.x} ${startY} C${from.x} ${mid - 18} ${to.x} ${mid + 18} ${to.x} ${endY}`;
}

export function QaNetworkEmptyStateV7() {
  ...
  const revealedDetailedCount = members.slice(4).reduce((count, _member, index) => {
    const progress = clamp(thirdGenerationProgress * 1.2 - index * 0.06, 0, 1);
    return count + (progress > 0.45 ? 1 : 0);
  }, 0);
  const hiddenCount = Math.max(0, TOTAL_COLLAPSED - revealedDetailedCount);
  const continuationOpacity = hiddenCount > 0 ? clamp(1 - thirdGenerationProgress * 0.34, 0.55, 1) : 0;
  ...
  const clampView = useCallback((next: View): View => {
    ...
    const minY = Math.min(-140, stageSize.height * 0.46 - WORLD_H * scale);
  ...
  const openContinuation = useCallback(() => {
    ...
    setSelectedId(null);
    setShowHint(false);
    focusWorldPoint({ x: 226, y: 612 }, Math.max(1.34, view.scale), stageSize.height * 0.4);
  ...
  const continuationCollectors = [
    'M110 510 C132 526 164 540 201 548',
    'M226 510 C226 525 226 538 226 548',
    'M342 510 C320 526 288 540 251 548',
  ];
  const continuationStem = 'M226 548 C226 557 226 564 226 571';

  return (
    <main className="qaPage">
      <section className="notice">
        <div><strong>NETWORK EMPTY STATE · QA V7</strong><span>누적 QA · Production과 완전히 분리</span></div>
        <small>Organic edges + correct +N semantics</small>
      </section>
        ...
        <div ref={stageRef} className="stage" onWheel={onWheel} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerEnd} onPointerCancel={onPointerEnd} onLostPointerCapture={onPointerEnd}>
          ...
          <div className={`world ${cameraMoving ? 'cameraMoving' : ''}`} style={{ width: WORLD_W, height: WORLD_H, marginLeft: -WORLD_W / 2, transform: `translate3d(${view.x}px,${view.y}px,0) scale(${view.scale})` }}>
            <svg className="edges" width={WORLD_W} height={WORLD_H} viewBox={`0 0 ${WORLD_W} ${WORLD_H}`} aria-hidden="true">
              <defs>
                <filter id="maskBlurV7" x="-120%" y="-120%" width="340%" height="340%"><feGaussianBlur stdDeviation="25" /></filter>
                <filter id="lineGlowV7" x="-160%" y="-160%" width="420%" height="420%"><feGaussianBlur stdDeviation="5" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
                <mask id="movingLightMaskV7" maskUnits="userSpaceOnUse" x="328" y="116" width="266" height="178">
                  <rect x="328" y="116" width="266" height="178" fill="black" />
                  <circle r="44" fill="white" filter="url(#maskBlurV7)"><animateMotion dur="5.3s" repeatCount="indefinite" path={emptyFlowPath} keyPoints="0;0;1;1" keyTimes="0;0.09;0.65;1" calcMode="linear" /></circle>
                </mask>
              </defs>
              ...
              <g className="flowIllumination" mask="url(#movingLightMaskV7)"><path d={emptyFlowPath} className="litGlow" vectorEffect="non-scaling-stroke" /><path d={emptyFlowPath} className="litCore" vectorEffect="non-scaling-stroke" /></g>
              ...
              <g className="collectorGroup" style={{ opacity: continuationOpacity }}>
              ...
              {deeperEdges.map(([fromId, toId]) => {
                ...
                return <path key={`${fromId}-${toId}`} d={childEdgePath(from, to)} className="branchEdge revealedEdge" style={{ opacity: thirdGenerationProgress * 0.47 }} vectorEffect="non-scaling-stroke" />;
              })}
            </svg>
            ...
            {hiddenCount > 0 ? <button type="button" className="continuation" style={{ left: 226, top: 606, opacity: continuationOpacity, pointerEvents: continuationOpacity > 0.2 ? 'auto' : 'none' }} data-network-control="true" onClick={openContinuation} aria-label={`${hiddenCount} more people below. Expand network`}>
              ...
              <b>+{hiddenCount}</b><small>more below</small>
            </button> : null}
            ...
            <div className="deeperHint" style={{ left: 226, top: 842, opacity: thirdGenerationProgress }} aria-hidden="true"><span>•••</span><b>more generations</b></div>
          </div>
      ...
      <section className="tips"><span><b>Organic edges</b>더 완만한 분기와 node gap</span><span><b>Collapsed Network</b>세 갈래 전체 아래의 +N</span><span><b>Available</b>기본선은 조용하고 빛만 흐름</span></section>
      ...
      <style jsx>{`
        ...
        @keyframes illuminationCycle{0%,7%{opacity:0}12%,62%{opacity:1}70%,100%{opacity:0}}@keyframes slotArrivalV7{0%,59%{transform:translate(-50%,0) scale(.97);opacity:0}66%{opacity:.46}74%{transform:translate(-50%,0) scale(1.1);opacity:0}100%{opacity:0}}@keyframes hintOut{0%,78%{opacity:1}100%{opacity:0}}
        ...
        @media(prefers-reduced-motion:reduce){.gestureHint,.flowIllumination,.slotArrival{animation:none}.flowIllumination{display:none}.world.cameraMoving{transition:none}.slotAvatar{box-shadow:0 0 0 6px rgba(244,183,40,.02),0 0 18px rgba(244,183,40,.04)}}
      `}</style>
    </main>
  );
}
