...
const ROOT_VIEW: View = { x: 0, y: 24, scale: 0.88 };
...
const members = [
  ...
  { id: 'a11', wallet: '0xA111...111', x: 70, y: 680, network: 2 },
  { id: 'a12', wallet: '0xA112...112', x: 152, y: 680, network: 1 },
  { id: 'a21', wallet: '0xA121...121', x: 224, y: 680, network: 1 },
  { id: 'a22', wallet: '0xA122...122', x: 298, y: 680, network: 1 },
  { id: 'a31', wallet: '0xA131...131', x: 372, y: 680, network: 0 },
];
...
export function QaNetworkEmptyStateV5() {
  ...
  const continuationOpacity = clamp(1 - thirdGenerationProgress * 0.94, 0, 1);
  ...
  const clampView = useCallback((next: View): View => {
    ...
    const horizontalReach = Math.max(150, WORLD_W * scale * 0.56);
    const minY = Math.min(-90, stageSize.height * 0.44 - WORLD_H * scale);
    const maxY = Math.max(150, stageSize.height * 0.33);
    return {
      scale,
  ...
  const animateTo = useCallback((next: View) => {
    ...
    cameraTimerRef.current = window.setTimeout(() => setCameraMoving(false), 470);
  }, [clampView]);
  ...
  const openContinuation = useCallback(() => {
    ...
    setSelectedId(null);
    setShowHint(false);
    focusWorldPoint({ x: 226, y: 590 }, Math.max(1.2, view.scale), stageSize.height * 0.39);
  ...
  const rootToActivePath = 'M380 145 C348 195 298 222 226 252';
  const rootToEmptyPath = 'M380 145 C414 195 466 222 534 252';

  return (
    <main className="qaPage">
      <section className="notice">
        <div><strong>NETWORK EMPTY STATE · QA V5</strong><span>샘플 데이터 · Production과 완전히 분리</span></div>
        <small>접힌 네트워크 + 탐색 복귀 개선</small>
      </section>
        ...
        >
          ...
          <div className={`world ${cameraMoving ? 'cameraMoving' : ''}`} style={{ width: WORLD_W, height: WORLD_H, marginLeft: -WORLD_W / 2, transform: `translate3d(${view.x}px,${view.y}px,0) scale(${view.scale})` }}>
            <svg className="edges" width={WORLD_W} height={WORLD_H} viewBox={`0 0 ${WORLD_W} ${WORLD_H}`} aria-hidden="true">
              <defs>
                <filter id="maskBlurV5" x="-120%" y="-120%" width="340%" height="340%"><feGaussianBlur stdDeviation="22" /></filter>
                <filter id="lineGlowV5" x="-160%" y="-160%" width="420%" height="420%"><feGaussianBlur stdDeviation="5.5" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
                <mask id="movingLightMaskV5" maskUnits="userSpaceOnUse" x="330" y="100" width="260" height="210">
                  <rect x="330" y="100" width="260" height="210" fill="black" />
                  <circle r="38" fill="white" filter="url(#maskBlurV5)"><animateMotion dur="4.8s" repeatCount="indefinite" path={rootToEmptyPath} keyPoints="0;0;1;1" keyTimes="0;0.08;0.68;1" calcMode="linear" /></circle>
                </mask>
              </defs>
              ...
              <g className="flowIllumination" mask="url(#movingLightMaskV5)"><path d={rootToEmptyPath} className="litGlow" vectorEffect="non-scaling-stroke" /><path d={rootToEmptyPath} className="litCore" vectorEffect="non-scaling-stroke" /></g>

              <path d="M226 322 C210 370 162 412 110 436" className="branchEdge" style={{ opacity: 0.32 + secondGenerationProgress * 0.28 }} vectorEffect="non-scaling-stroke" />
              <path d="M226 322 C226 372 226 408 226 436" className="branchEdge" style={{ opacity: 0.32 + secondGenerationProgress * 0.28 }} vectorEffect="non-scaling-stroke" />
              <path d="M226 322 C242 370 292 412 342 436" className="branchEdge" style={{ opacity: 0.32 + secondGenerationProgress * 0.28 }} vectorEffect="non-scaling-stroke" />

              <path d="M110 506 C126 538 166 554 201 568" className="continuationEdge" style={{ opacity: continuationOpacity * .32 }} vectorEffect="non-scaling-stroke" />
              <path d="M226 506 L226 566" className="continuationEdge" style={{ opacity: continuationOpacity * .42 }} vectorEffect="non-scaling-stroke" />
              <path d="M342 506 C326 538 286 554 251 568" className="continuationEdge" style={{ opacity: continuationOpacity * .32 }} vectorEffect="non-scaling-stroke" />

              {deeperEdges.map(([fromId, toId]) => {
                const from = members.find((member) => member.id === fromId)!;
                const to = members.find((member) => member.id === toId)!;
                return <line key={`${fromId}-${toId}`} x1={from.x} y1={from.y + 34} x2={to.x} y2={to.y - 33} className="branchEdge revealedEdge" style={{ opacity: thirdGenerationProgress * 0.58 }} vectorEffect="non-scaling-stroke" />;
              })}
            </svg>

            <button type="button" className="person root" style={{ left: 380, top: 108 }} data-network-control="true" onClick={() => setSelectedId(null)}><span className="avatar">●</span><b>YOU</b><small>1 Direct · 1 Open Slot</small></button>
            ...
            {members.slice(4).map((member, index) => {
              const progress = clamp(thirdGenerationProgress * 1.2 - index * 0.06, 0, 1);
              return <button key={member.id} type="button" className="person child mini" style={{ left: member.x, top: member.y, opacity: progress, transform: `translate(-50%,-50%) scale(${0.74 + progress * 0.26})`, pointerEvents: progress > 0.45 ? 'auto' : 'none' }} data-network-control="true" onClick={() => setSelectedId(member.id)}><span className="avatar">●</span><b>{member.wallet}</b><small>{member.network} network</small></button>;
            })}

            <div className="deeperHint" style={{ left: 226, top: 810, opacity: thirdGenerationProgress }} aria-hidden="true"><span>•••</span><b>more generations</b></div>
          </div>
          ...
          {selected ? <aside className="slotCard" data-network-control="true"><div><span className="cardAvatar">●</span><div><strong>{selected.wallet}</strong><small>{selected.network} network members</small></div><button type="button" aria-label="닫기" onClick={() => setSelectedId(null)}>×</button></div><p>실제 사용자 노드입니다. 접힌 네트워크 표시와 역할을 분리했습니다.</p></aside> : null}
