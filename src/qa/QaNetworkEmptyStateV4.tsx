...
export function QaNetworkEmptyStateV4() {
  ...
  return (
    <main className="qaPage">
      <section className="notice">
        <div><strong>NETWORK EMPTY STATE · QA V4</strong><span>샘플 데이터 · Production과 완전히 분리</span></div>
        <small>선 자체의 밝기가 흐르는 표현</small>
      </section>
      ...
      <section className="networkShell">
        ...
        <div ref={stageRef} className="stage" onWheel={onWheel} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerEnd} onPointerCancel={onPointerEnd} onPointerLeave={onPointerEnd}>
          ...
          <div className="world" style={{ width: WORLD_W, height: WORLD_H, marginLeft: -WORLD_W / 2, transform: `translate3d(${view.x}px,${view.y}px,0) scale(${view.scale})` }}>
            <svg className="edges" width={WORLD_W} height={WORLD_H} viewBox={`0 0 ${WORLD_W} ${WORLD_H}`} aria-hidden="true">
              <defs>
                <filter id="maskBlurV4" x="-120%" y="-120%" width="340%" height="340%"><feGaussianBlur stdDeviation="22" /></filter>
                <filter id="lineGlowV4" x="-160%" y="-160%" width="420%" height="420%"><feGaussianBlur stdDeviation="5.5" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
                <mask id="movingLightMaskV4" maskUnits="userSpaceOnUse" x="330" y="100" width="260" height="210">
                  <rect x="330" y="100" width="260" height="210" fill="black" />
                  <circle r="38" fill="white" filter="url(#maskBlurV4)">
                    <animateMotion dur="4.8s" repeatCount="indefinite" path={rootToEmptyPath} keyPoints="0;0;1;1" keyTimes="0;0.08;0.68;1" calcMode="linear" />
                  </circle>
                </mask>
              </defs>
              ...
              <g className="flowIllumination" mask="url(#movingLightMaskV4)">
                <path d={rootToEmptyPath} className="litGlow" vectorEffect="non-scaling-stroke" />
                <path d={rootToEmptyPath} className="litCore" vectorEffect="non-scaling-stroke" />
              </g>
