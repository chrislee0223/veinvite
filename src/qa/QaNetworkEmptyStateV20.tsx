...
const LANE_STORAGE_PREFIX = 'veinvite:network:v20:';
...
function countLayoutConflicts(nodes: Map<string, LayoutNode>, edges: Edge[]) {
  ...
  for (let i = 0; i < list.length; i += 1) {
    for (let j = i + 1; j < list.length; j += 1) {
      const a = list[i];
      const b = list[j];
      if (a.localDepth === b.localDepth && Math.abs(a.x - b.x) < 88 && Math.abs(a.y - b.y) < 70) count += 1;
    }
  }
  for (let i = 0; i < edges.length; i += 1) {
    ...
    const edge = edges[i];
    const a = { x: edge.parent.x, y: edge.parent.y + 58 };
    const b = { x: edge.child.x, y: edge.child.y - 37 };
    for (const node of list) {
      if (node.wallet === edge.parent.wallet || node.wallet === edge.child.wallet) continue;
      if (pointToSegmentDistance({ x: node.x, y: node.y }, a, b) < 42) count += 1;
    }
  }
  return count;
}
...
async function fetchNetworkApi(rootWallet: string, focusWallet: string, signal: AbortSignal): Promise<ApiPayload> {
  ...
  const normalized: ApiPayload = {
    ...
    breadcrumb: Array.isArray(payload.breadcrumb) ? payload.breadcrumb.filter(validWallet).map(keyWallet) : [keyWallet(payload.focusWallet)],
    children: stableSortChildren(payload.children),
    depthLimitReached: Boolean(payload.depthLimitReached),
  };
...
export function QaNetworkEmptyStateV20() {
  ...
  useEffect(() => { viewedRootRef.current = viewedRoot; }, [viewedRoot]);
  ...
  const loadPayload = useCallback((focusWallet: string, force = false): Promise<ApiPayload> => {
    ...
    const promise = fetchNetworkApi(requestRoot, focus, controller.signal)
      .then((payload) => {
        if (generationRef.current !== requestGeneration || authWalletRef.current !== requestRoot) throw new StaleRequestError();
        writeCache(focus, payload);
        setLoadError('');
        return payload;
      })
      .catch((error: unknown) => {
        if (error instanceof StaleRequestError || (error instanceof DOMException && error.name === 'AbortError')) throw error;
    ...
    networkRequestsRef.current.set(requestKey, { wallet: requestRoot, controller, promise });
    return promise;
  }, [writeCache]);
  ...
  const loadSlots = useCallback((force = false): Promise<ReferralLinkPayload> => {
    ...
    if (!force && slotAvailabilityRef.current) {
      return Promise.resolve({
        referralLink: referralKeyRef.current ? { key: referralKeyRef.current, createdAt: '', slotsAvailable: slotAvailabilityRef.current.slotsAvailable } : null,
  ...
  const refreshData = useCallback(async (immediate = false) => {
    ...
    await Promise.allSettled([...new Set([root, currentView])].map((target) => loadPayload(target, true)));
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        const parsed = JSON.parse(raw) as { members?: Array<[string, number]>; slots?: Array<[number, number]> };
        if (Array.isArray(parsed.members)) {
          memberLaneRef.current = new Map(parsed.members.filter(([w, lane]) => validWallet(w) && Number.isInteger(lane)).slice(0, 160).map(([w, lane]) => [keyWallet(w), lane]));
        }
        if (Array.isArray(parsed.slots)) {
          slotLaneRef.current = new Map(parsed.slots.filter(([slot, lane]) => Number.isInteger(slot) && slot > 0 && Number.isInteger(lane)).slice(0, 40));
        }
      }
    } catch {}
    ...
    void (async () => {
      try {
        ...
        if (generationRef.current === serial && authWalletRef.current === authWallet) setBooting(false);
      }
    })();
  ...
  useEffect(() => {
    ...
    for (const slotId of [...slots.keys()]) {
      if (!currentSlotIds.has(slotId)) { slots.delete(slotId); changed = true; }
    }
    for (const slot of slotAvailability?.slots ?? []) {
      ...
      if (slotLane !== undefined && !members.has(invitee)) { members.set(invitee, slotLane); changed = true; }
    }
    ...
    const nextOuterLane = (preferLeft: boolean) => {
      ...
      return preferLeft ? min - 1 : max + 1;
    };
    for (const slot of slotAvailability?.slots ?? []) {
      const invitee = slot.inviteeWallet ? keyWallet(slot.inviteeWallet) : null;
      if (invitee && members.has(invitee)) {
        const memberLane = members.get(invitee)!;
        if (slots.get(slot.slot) !== memberLane) { slots.set(slot.slot, memberLane); changed = true; }
        continue;
      }
      const lane = slots.get(slot.slot);
      const occupiedByMember = lane !== undefined && new Set(members.values()).has(lane);
      if (lane === undefined || (slot.state === 'AVAILABLE' && occupiedByMember)) {
        slots.set(slot.slot, nextOuterLane(slot.slot % 2 === 1));
        changed = true;
      }
    }
    if (!changed) return;
    setLaneVersion((value) => value + 1);
    try {
      window.localStorage.setItem(`${LANE_STORAGE_PREFIX}${authWallet}`, JSON.stringify({
        members: [...members.entries()].slice(0, 160),
        slots: [...slots.entries()].slice(0, 40),
      }));
    } catch {}
  }, [authRootPayload, authWallet, slotAvailability]);
  ...
  useEffect(() => { if (zoomLevel === 0 && !branchAutoEnabled) setBranchAutoEnabled(true); }, [branchAutoEnabled, zoomLevel]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onReturn = () => { if (document.visibilityState === 'visible') void refreshData(false); };
    const onReferralUpdated = () => { if (document.visibilityState === 'visible') void refreshData(true); };
  ...
  const rootRevealCap = ROOT_REVEAL_BY_ZOOM[zoomLevel] ?? 9;
  const childRevealCap = CHILD_REVEAL_BY_ZOOM[zoomLevel] ?? 9;
  const rootGroup = rootChildren.slice(safeRootPage * ROOT_PAGE_SIZE, safeRootPage * ROOT_PAGE_SIZE + ROOT_PAGE_SIZE);
  const rootVisibleChildren = rootGroup.slice(0, rootRevealCap);
  const rootIndexByWallet = useMemo(() => new Map(rootChildren.map((child, index) => [child.wallet, index])), [rootChildren]);

  useEffect(() => { if (rootPage !== safeRootPage) setRootPage(safeRootPage); }, [rootPage, safeRootPage]);
  ...
  const rootLaneFor = useCallback((walletAddress: string) => {
    ...
    return stableOffset(rootIndexByWallet.get(walletKey) ?? 0);
  }, [isOwnNetwork, rootIndexByWallet]);

  const pageChildrenFor = useCallback((parentWallet: string, revealOverride?: number) => {
    const payload = payloads.get(keyWallet(parentWallet));
    const reveal = revealOverride ?? childRevealCap;
    if (!payload || reveal <= 0) return [] as ApiChild[];
    const page = pageByParent.get(keyWallet(parentWallet)) ?? 0;
    const start = page * CHILD_PAGE_SIZE;
    return payload.children.slice(start, start + CHILD_PAGE_SIZE).slice(0, reveal);
  }, [childRevealCap, pageByParent, payloads]);

  useEffect(() => {
    if (zoomLevel < 1 || !branchAutoEnabled || activeBranch || !rootVisibleChildren.length) return;
    const candidate = rootVisibleChildren.find((child) => child.direct > 0);
    if (!candidate) return;
    setActiveBranch(candidate.wallet);
    setFocusPath([candidate.wallet]);
  }, [activeBranch, branchAutoEnabled, rootVisibleChildren, zoomLevel]);
  ...
  useEffect(() => {
    ...
    const children = pageChildrenFor(activeBranch);
    const current = focusPath[1];
    if (current && children.some((child) => child.wallet === current)) {
      void loadPayload(current).catch(() => undefined);
      return;
    }
    const candidate = children.find((child) => child.direct > 0);
    if (!candidate) return;
  ...
  useEffect(() => {
    ...
    const children = pageChildrenFor(parent);
    ...
    const candidate = children.find((child) => child.direct > 0);
    if (!candidate) return;
    setFocusPath((path) => [path[0], parent, candidate.wallet]);
    void loadPayload(candidate.wallet).catch(() => undefined);
  }, [focusPath, loadPayload, pageChildrenFor, zoomLevel]);

  const relativeLayout = useMemo<LayoutResult>(() => {
    const build = (gapScale: number, revealOverride?: number) => {
      ...
      rootVisibleChildren.forEach((child) => {
        nodes.set(child.wallet, { ...child, parentWallet: viewedRoot, x: rootLaneFor(child.wallet) * DIRECT_GAP, y: ROOT_CHILD_Y, localDepth: 1, branch: child.wallet });
      });
      const placePage = (parentWallet: string, localDepth: number) => {
        const parent = nodes.get(parentWallet);
        if (!parent || localDepth >= MAX_LOCAL_DEPTH) return;
        const children = pageChildrenFor(parentWallet, revealOverride);
        const gap = Math.round(CHILD_GAP * gapScale * (localDepth >= 3 ? .94 : 1));
    ...
    const attempts: Array<{ gap: number; reveal?: number }> = [
      { gap: 1 },
      { gap: 1.18 },
      { gap: 1.42 },
      { gap: 1.72 },
    ...
    for (const attempt of attempts) {
      const built = build(attempt.gap, attempt.reveal);
      const conflicts = countLayoutConflicts(built.nodes, built.edges);
      const result: LayoutResult = {
        ...built,
        conflicts,
        autoExpanded: attempt.gap > 1,
        trimmed: attempt.reveal !== undefined && attempt.reveal < childRevealCap,
        gapScale: attempt.gap,
      };
      fallback = result;
      if (!conflicts) return result;
    }
    return fallback ?? { nodes: new Map(), edges: [], conflicts: 0, autoExpanded: false, trimmed: false, gapScale: 1 };
  }, [activeBranch, childRevealCap, focusPath, pageChildrenFor, rootLaneFor, rootPayload, rootVisibleChildren, viewedRoot, zoomLevel]);

  const canvas = useMemo(() => {
    const values = [...relativeLayout.nodes.values()];
    const slotRelativeXs = isOwnNetwork ? [...slotLaneRef.current.values()].map((lane) => lane * DIRECT_GAP) : [];
    const maxAbsX = Math.max(650, ...values.map((node) => Math.abs(node.x) + 150), ...slotRelativeXs.map((x) => Math.abs(x) + 150));
    const maxY = Math.max(1160, ...values.map((node) => node.y + 210));
    const worldW = Math.ceil((maxAbsX + 180) * 2);
    const worldH = Math.ceil(maxY + 180);
    const rootX = worldW / 2;
    const nodes = new Map<string, LayoutNode>();
    relativeLayout.nodes.forEach((node, walletKey) => nodes.set(walletKey, { ...node, x: node.x + rootX }));
    const edges = relativeLayout.edges.flatMap(({ parent, child }) => {
      const rp = nodes.get(parent.wallet);
      const rc = nodes.get(child.wallet);
      return rp && rc ? [{ parent: rp, child: rc }] : [];
    });
    return { worldW, worldH, rootX, nodes, edges };
  }, [isOwnNetwork, laneVersion, relativeLayout]);

  const { worldW, worldH, rootX } = canvas;
  const layout = canvas.nodes;

  const clampView = useCallback((next: View): View => {
    const scale = clamp(next.scale, MIN_SCALE, MAX_SCALE);
    const horizontalReach = Math.max(360, worldW * scale * .58);
    const minY = Math.min(-450, stageSize.height * .52 - worldH * scale);
  ...
  }, [stageSize.height, worldH, worldW]);
  ...
  const slotAnchors = useMemo<SlotAnchor[]>(() => {
    ...
    return slotAvailability.slots.flatMap((slot) => {
      ...
      return [{ serverSlot: slot.slot, lane, x, y: ROOT_CHILD_Y, path: slotPath(rootX, x, ROOT_CHILD_Y), state: slot.state, inviteeWallet: slot.inviteeWallet ? keyWallet(slot.inviteeWallet) : null }];
    });
  }, [isOwnNetwork, laneVersion, rootX, slotAvailability]);

  const slotAnchorByWallet = useMemo(() => {
    const map = new Map<string, SlotAnchor>();
    slotAnchors.forEach((anchor) => { if (anchor.inviteeWallet) map.set(anchor.inviteeWallet, anchor); });
    return map;
  }, [slotAnchors]);
  const availableAnchors = useMemo(() => slotAnchors.filter((anchor) => anchor.state === 'AVAILABLE'), [slotAnchors]);
  const pendingAnchors = useMemo(() => {
    ...
    return slotAnchors.filter((anchor) => anchor.state !== 'AVAILABLE' && (!anchor.inviteeWallet || !childWallets.has(anchor.inviteeWallet)));
  ...
  const visibleEdges = useMemo(() => canvas.edges.filter(({ parent, child }) => visibleWallets.has(parent.wallet) && visibleWallets.has(child.wallet)), [canvas.edges, visibleWallets]);
  const semanticChildrenByParent = useMemo(() => {
    const counts = new Map<string, number>();
    canvas.edges.forEach(({ parent }) => counts.set(parent.wallet, (counts.get(parent.wallet) ?? 0) + 1));
    return counts;
  }, [canvas.edges]);
  const continuationNodes = useMemo(() => visibleNodes.filter((node) => {
    ...
    return (semanticChildrenByParent.get(node.wallet) ?? 0) < node.direct || payload.depthLimitReached;
  ...
  const selectedPageCount = selectedPayload ? Math.max(1, Math.ceil(selectedPayload.children.length / CHILD_PAGE_SIZE)) : 1;
  const actionsBusy = Boolean(actionKey);
  const canvasExpanded = worldW > 1660 || worldH > 1550;
  ...
  const focusNode = useCallback((walletToFocus: string) => {
    void runAction(`focus:${walletToFocus}`, async () => {
      ...
      if (node.localDepth >= MAX_LOCAL_DEPTH) {
        ...
        setViewedRoot(node.wallet);
        viewedRootRef.current = node.wallet;
        setRootPage(0);
        setPageByParent(new Map());
        setActiveBranch(null);
        setFocusPath([]);
        ...
        setSelectedWallet(null);
        setZoomLevel(1);
        animateTo({ ...ROOT_VIEW, scale: 1.16 });
        return;
      }
      setSelectedWallet(null);
      setShowHint(false);
      ...
      animateTo({ x: -(node.x - worldW / 2) * scale, y: stageSize.height * .31 - node.y * scale, scale });
    });
  }, [activeBranch, animateTo, branchAutoEnabled, focusPath, layout, loadPayload, pageByParent, rootPage, runAction, stageSize.height, viewedRoot, worldW]);
  ...
  }, [focusNode, layout, loadPayload, runAction]);
  ...
  }, [activeBranch, animateTo, authWallet, branchAutoEnabled, focusPath, loadPayload, pageByParent, rootPage, runAction, viewedRoot]);
  ...
  const changeRootPage = useCallback((delta: number) => {
    ...
    setBranchAutoEnabled(true);
    setSelectedWallet(null);
    animateTo({ ...viewRef.current, x: 0 });
  }, [animateTo, rootPageCount, safeRootPage]);
  ...
  if (!authWallet) {
    return (
      <main className="qaPage">
        <section className="notice"><div><strong>DEEP RESILIENCE · QA V20</strong><span>v19 누적 · deep handoff · collision fallback · dynamic canvas</span></div></section>
        <section className="connectCard">
          <span>NETWORK</span><h1>Connect your wallet</h1>
          <p>깊은 세대와 대형 네트워크까지 실제 데이터로 검증합니다.</p>
          <button type="button" onClick={openWallet} disabled={isWalletActionPending}>{isWalletActionPending ? 'Opening…' : 'Connect wallet'}</button>
        </section>
        <style jsx>{`
          .qaPage{min-height:100svh;padding:12px 0 28px;background:#080807;color:#f3efe6}.notice,.connectCard{width:min(calc(100vw - 20px),590px);box-sizing:border-box;margin:auto}.notice{margin-bottom:8px;padding:9px 11px;border:1px solid rgba(244,183,40,.12);border-radius:13px;background:rgba(244,183,40,.035)}.notice div{display:grid;gap:2px}.notice strong{color:#d7ac42;font-size:.56rem;letter-spacing:.08em}.notice span{color:#7f786d;font-size:.5rem}.connectCard{margin-top:80px;padding:26px;border:1px solid rgba(255,255,255,.06);border-radius:20px;background:#0b0b09;text-align:center}.connectCard>span{color:#817353;font-size:.5rem;font-weight:900;letter-spacing:.14em}.connectCard h1{margin:8px 0;font-size:1.1rem}.connectCard p{margin:0 auto 18px;max-width:330px;color:#827c72;font-size:.7rem;line-height:1.6}.connectCard button{height:40px;padding:0 16px;border:1px solid rgba(244,183,40,.2);border-radius:11px;background:rgba(244,183,40,.08);color:#d1ad54;font-size:.65rem}
        `}</style>
      </main>
    );
  }
  ...
  return (
    <main className="qaPage">
      <section className="notice">
        <div><strong>DEEP RESILIENCE · QA V20</strong><span>v19 누적 · unlimited handoff · collision fallback · auto canvas bounds</span></div>
        <div className="qaActions">
          <small className={relativeLayout.conflicts ? 'bad' : 'good'}>{relativeLayout.conflicts} conflicts</small>
          {relativeLayout.autoExpanded ? <small>gap ×{relativeLayout.gapScale.toFixed(2)}</small> : null}
          {relativeLayout.trimmed ? <small>continuation fallback</small> : null}
          {canvasExpanded ? <small>canvas expanded</small> : null}
          {rootPayload?.depthLimitReached ? <small>server depth limit</small> : null}
      ...
      {authError ? <section className="errorStrip"><span>{authError}</span><button type="button" onClick={verifyAndRetry} disabled={actionsBusy}>{actionKey === 'verify' ? 'Verifying…' : 'Verify & retry'}</button></section> : null}
      {(loadError || slotError) && rootPayload ? <section className="staleStrip"><span>{loadError || slotError} · 기존 데이터는 유지했습니다.</span><button type="button" onClick={() => void refreshData(true)} disabled={actionsBusy}>Retry</button></section> : null}
      ...
      <section className="networkShell">
        <header className="topBar">
          ...
          <div className="totals">
            <span className="metric"><b>{rootPayload ? rootPayload.summary.network : '—'}</b><em>Members</em></span><i />
            {isOwnNetwork ? <span className="metric"><b>{slotCount ?? '—'}</b><em>Open slots</em></span> : <span className="metric"><b>{rootPayload?.summary.direct ?? '—'}</b><em>Direct</em></span>}
          </div>
        </header>

        {!isOwnNetwork && currentBreadcrumb.length ? (
          <div className="breadcrumb" data-network-control="true">
            {currentBreadcrumb.map((crumb, index) => <span key={`${crumb}-${index}`}><button type="button" disabled={actionsBusy} onClick={() => reroot(crumb)}>{index === 0 ? 'YOU' : shortWallet(crumb, authWallet)}</button>{index < currentBreadcrumb.length - 1 ? <i>›</i> : null}</span>)}
          </div>
        ) : null}
        ...
        <div ref={stageRef} className="stage" onWheel={onWheel} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerEnd} onPointerCancel={onPointerEnd} onLostPointerCapture={onPointerEnd}>
          ...
          <div className="hud" data-network-control="true"><span>{loadingWallets.size || booting ? 'Loading…' : zoomLevel === 0 ? '전체 구조' : activeBranch ? '가지 고정' : '상세 보기'}</span><small>{Math.round(view.scale * 100)}%</small></div>
          {rootPageCount > 1 ? <div className="rootPager" data-network-control="true"><button type="button" onClick={() => changeRootPage(-1)} disabled={actionsBusy}>‹</button><span>{safeRootPage + 1}/{rootPageCount}</span><button type="button" onClick={() => changeRootPage(1)} disabled={actionsBusy}>›</button></div> : null}
          {history.length ? <button type="button" className="backNetwork" data-network-control="true" disabled={actionsBusy} onClick={goBack}>← <span>Back</span></button> : null}
          {activeBranch ? <button type="button" className="clearFocus" data-network-control="true" onClick={() => { setBranchAutoEnabled(false); setActiveBranch(null); setFocusPath([]); }}>전체 가지 보기</button> : null}
          {showHint ? <div className="gestureHint" data-network-control="true"><b>↗</b><span>끝 세대에서도 계속 내려갈 수 있습니다.<br />겹치면 자동으로 간격을 조정합니다.</span></div> : null}
          {!rootPayload && !loading ? <div className="emptyLoad" data-network-control="true"><b>{loadError || authError || 'Network data is unavailable.'}</b><button type="button" onClick={() => void refreshData(true)}>Retry</button></div> : null}

          <div className={`world ${cameraMoving ? 'cameraMoving' : ''}`} style={{ width: worldW, height: worldH, marginLeft: -worldW / 2, transform: `translate3d(${view.x}px,${view.y}px,0) scale(${view.scale})` }}>
            <svg className="edges" width={worldW} height={worldH} viewBox={`0 0 ${worldW} ${worldH}`} aria-hidden="true">
              <defs>
                <linearGradient id="continuationFadeV20" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="rgba(222,209,176,.46)"/><stop offset="64%" stopColor="rgba(211,198,165,.22)"/><stop offset="100%" stopColor="rgba(211,198,165,0)"/></linearGradient>
                <filter id="slotGlowV20" x="-160%" y="-160%" width="420%" height="420%"><feGaussianBlur stdDeviation="5.5"/></filter>
                {availableAnchors.map((slot, index) => {
                  const dx = slot.x - rootX;
                  const dy = slot.y - (ROOT_Y + 44);
                  return <linearGradient key={slot.serverSlot} id={`slotFlowV20-${slot.serverSlot}`} gradientUnits="userSpaceOnUse" x1={rootX} y1={ROOT_Y + 44} x2={slot.x} y2={slot.y - 38}>
              ...
              {visibleEdges.map(({ parent, child }) => {
                const anchor = isOwnNetwork && parent.wallet === viewedRoot ? slotAnchorByWallet.get(child.wallet) : null;
              ...
              {pendingAnchors.map((anchor) => <path key={`pending-${anchor.serverSlot}`} d={anchor.path} className="pendingEdge" vectorEffect="non-scaling-stroke"/>)}
              {availableAnchors.map((slot) => <g key={`slot-${slot.serverSlot}`}><path d={slot.path} className="availableBase" vectorEffect="non-scaling-stroke"/><path d={slot.path} className="availableFlow" stroke={`url(#slotFlowV20-${slot.serverSlot})`} vectorEffect="non-scaling-stroke"/></g>)}
            </svg>

            {visibleNodes.map((node, index) => {
              const isRoot = node.wallet === viewedRoot;
              const nodeLoading = loadingWallets.has(node.wallet);
              return <button key={node.wallet} type="button" className={`person ${isRoot ? 'root' : 'child'} ${node.localDepth >= 2 ? 'mini' : ''} ${recentArrivalWallet === node.wallet ? 'arriving' : ''}`} style={{ left: node.x, top: node.y, animationDelay: reduceMotion ? undefined : `${Math.min(index, 8) * 34 + 70}ms` }} data-node-wallet={node.wallet}>
                <span className="avatar">●</span><b>{isRoot && node.wallet === authWallet ? 'YOU' : shortWallet(node.wallet, authWallet)}</b><small>{nodeLoading ? 'Loading…' : node.direct ? `${node.network} network · ${node.direct} direct` : 'Leaf'}</small>
              </button>;
            })}
            {pendingAnchors.map((anchor) => <div key={`pending-node-${anchor.serverSlot}`} className="pendingSlot" style={{ left: anchor.x, top: anchor.y }}><span className="slotAvatar pending">…</span><b>{anchor.state === 'PENDING' ? 'Pending' : 'Joining'}</b><small>Invite in progress</small></div>)}
            {availableAnchors.map((slot, index) => <button key={slot.serverSlot} type="button" className="emptySlot" style={{ left: slot.x, top: slot.y }} data-network-control="true" onClick={() => setInviteNotice(referralKey ? '친구 초대에 사용할 수 있는 슬롯입니다.' : '초대 링크 생성 전에도 사용할 수 있는 슬롯입니다.')}><span className="slotAvatar" style={{ animationDelay: `${3.9 + index * 4}s` }}><i>+</i></span><b>Available</b><small>Invite slot</small></button>)}
          </div>

          {selected ? (
            <aside className="slotCard" data-network-control="true" aria-busy={actionsBusy}>
              <div><span className="cardAvatar">●</span><div><strong>{selected.wallet === authWallet ? 'YOU' : shortWallet(selected.wallet, authWallet)}</strong><small>{selected.direct} direct · {selected.network} network</small></div><button type="button" disabled={actionsBusy} onClick={() => setSelectedWallet(null)}>×</button></div>
              <p>{selected.localDepth === 0 ? '현재 보고 있는 네트워크의 기준점입니다.' : selected.direct ? selected.localDepth >= MAX_LOCAL_DEPTH ? '화면 끝에 도달했습니다. 이 사람을 새 기준점으로 열어 다음 세대를 계속 볼 수 있습니다.' : selectedPayload?.depthLimitReached ? '서버 읽기 한계에 도달했습니다. 이 사람을 기준으로 다시 열어 더 깊이 탐색할 수 있습니다.' : '이 가지는 사용자가 바꾸기 전까지 고정됩니다.' : '현재 서버 데이터 기준 leaf 노드입니다.'}</p>
              {selected.direct > 0 && selected.localDepth > 0 ? <div className="cardActions">
                <button type="button" className="branchCta" disabled={actionsBusy} onClick={() => focusNode(selected.wallet)}>{actionKey === `focus:${selected.wallet}` ? 'Loading…' : selected.localDepth >= MAX_LOCAL_DEPTH ? '계속 내려가기' : '이 가지 펼치기'}</button>
                {selected.localDepth < MAX_LOCAL_DEPTH && selectedPageCount > 1 ? <button type="button" className="moreCta" disabled={actionsBusy} onClick={() => showNextConnections(selected.wallet)}>{actionKey === `page:${selected.wallet}` ? 'Loading…' : '다음 연결 보기'}</button> : null}
                {selected.wallet !== viewedRoot && selected.localDepth < MAX_LOCAL_DEPTH ? <button type="button" className="networkCta" disabled={actionsBusy} onClick={() => reroot(selected.wallet)}>{actionKey === `reroot:${selected.wallet}` ? 'Loading…' : '이 네트워크 보기'}</button> : null}
              </div> : null}
            </aside>
          ) : null}
          {inviteNotice ? <div className="inviteToast" role="status" data-network-control="true">{inviteNotice}</div> : null}
          <div className="controls" data-network-control="true"><button type="button" className="youButton" disabled={actionsBusy} onClick={returnToYou}><span>◎</span><b>{actionKey === 'you' ? '…' : 'YOU'}</b></button><button type="button" className="zoomControl" onClick={() => commitView({ ...viewRef.current, scale: viewRef.current.scale + .16 })}>+</button><button type="button" className="zoomControl" onClick={() => commitView({ ...viewRef.current, scale: viewRef.current.scale - .16 })}>−</button></div>
        </div>
      </section>

      <section className="tips"><span><b>Deep handoff</b>4단계 끝에서도 기준점을 넘겨 계속 아래 세대를 탐색</span><span><b>Collision fallback</b>겹치면 간격을 단계적으로 확대하고 마지막엔 continuation으로 안전하게 축약</span><span><b>Dynamic canvas</b>실제 배치 범위를 계산해 가로·세로 캔버스를 자동 확장</span></section>

      <style jsx>{`
        .qaPage{min-height:100svh;padding:12px 0 28px;background:#080807;color:#f3efe6}.notice,.networkShell,.tips,.errorStrip,.staleStrip{width:min(calc(100vw - 20px),590px);box-sizing:border-box;margin-left:auto;margin-right:auto}.notice{margin-bottom:8px;padding:9px 11px;border:1px solid rgba(244,183,40,.12);border-radius:13px;background:rgba(244,183,40,.035);display:flex;align-items:center;justify-content:space-between;gap:10px}.notice>div:first-child{display:grid;gap:2px}.notice strong{color:#d7ac42;font-size:.56rem;letter-spacing:.08em}.notice span,.notice small{color:#7f786d;font-size:.5rem}.qaActions{display:flex;align-items:center;gap:6px;flex-wrap:wrap;justify-content:flex-end}.qaActions small.good{color:#769c6e}.qaActions small.bad{color:#d17a68}.qaActions small.live{color:#6fae7d}.qaActions button{height:25px;padding:0 7px;border:1px solid rgba(244,183,40,.16);border-radius:8px;background:rgba(244,183,40,.055);color:#c7a451;font-size:.45rem}button:disabled{opacity:.45;cursor:default}.errorStrip,.staleStrip{margin-bottom:8px;padding:8px 10px;border-radius:10px;display:flex;align-items:center;justify-content:space-between;gap:10px;font-size:.5rem}.errorStrip{border:1px solid rgba(209,122,104,.18);background:rgba(209,122,104,.05);color:#c9897c}.staleStrip{border:1px solid rgba(212,175,86,.16);background:rgba(212,175,86,.04);color:#a9915c}.errorStrip button,.staleStrip button{padding:5px 7px;border:1px solid currentColor;border-radius:7px}.networkShell{overflow:hidden;border:1px solid rgba(255,255,255,.055);border-radius:20px;background:#0b0b09}.topBar{min-height:62px;padding:8px 14px;display:flex;align-items:center;justify-content:space-between;gap:12px;border-bottom:1px solid rgba(255,255,255,.045)}.titleBlock{display:grid;gap:2px}.titleBlock>span{color:#817353;font-size:.5rem;font-weight:900;letter-spacing:.14em}.titleBlock h1{margin:0;font-size:.91rem}.totals{display:flex;align-items:center;gap:7px}.metric{display:flex;align-items:baseline;gap:3px}.metric b{font-size:.66rem}.metric em{font-style:normal;color:#6f6a62;font-size:.42rem}.totals i{width:1px;height:12px;background:rgba(255,255,255,.08)}.breadcrumb{min-height:29px;padding:0 12px;display:flex;align-items:center;gap:4px;overflow:auto;border-bottom:1px solid rgba(255,255,255,.035)}.breadcrumb span{display:flex;align-items:center;gap:4px;flex:0 0 auto}.breadcrumb button{color:#817b72;font-size:.45rem}.breadcrumb span:last-child button{color:#c2b59a}.breadcrumb i{color:#4e4a43}.stage{position:relative;height:clamp(570px,73svh,760px);overflow:hidden;touch-action:none;user-select:none;-webkit-user-select:none;cursor:grab;background:radial-gradient(circle at 50% 19%,rgba(244,183,40,.055),transparent 34%),#0b0b09}.stage:active{cursor:grabbing}.ambient{position:absolute;inset:0;pointer-events:none;background-image:radial-gradient(circle,rgba(255,255,255,.045) 1px,transparent 1px);background-size:28px 28px;mask-image:linear-gradient(to bottom,rgba(0,0,0,.42),transparent 90%)}.world{position:absolute;left:50%;top:0;transform-origin:50% 0;will-change:transform;transition:none}.world.cameraMoving{transition:transform .46s cubic-bezier(.2,.72,.25,1)}.edges{position:absolute;inset:0;overflow:visible;pointer-events:none}.treeEdge{fill:none;stroke:rgba(211,198,165,.31);stroke-width:1.05;stroke-linecap:round;stroke-linejoin:round;stroke-dasharray:1 0;animation:edgeReveal .28s ease-out both}.treeEdge.depth2{stroke:rgba(211,198,165,.27);stroke-width:.96}.treeEdge.depth3{stroke:rgba(211,198,165,.21);stroke-width:.88}.treeEdge.depth4{stroke:rgba(211,198,165,.17);stroke-width:.8}.continuationTail{fill:none;stroke:url(#continuationFadeV20);stroke-width:1.13;stroke-linecap:round;animation:tailReveal .34s ease-out both}.pendingEdge{fill:none;stroke:rgba(211,198,165,.16);stroke-width:1;stroke-dasharray:4 5}.availableBase{fill:none;stroke:rgba(244,183,40,.085);stroke-width:.85;stroke-linecap:round}.availableFlow{fill:none;stroke-width:2.7;stroke-linecap:round;filter:url(#slotGlowV20);opacity:.9}.person,.emptySlot,.pendingSlot{position:absolute;transform:translate(-50%,-50%);display:grid;justify-items:center;gap:3px;border:0;background:transparent;text-align:center;white-space:nowrap;touch-action:none}.person{min-width:86px;color:#d8d2c7;animation:nodeReveal .32s ease-out both}.person .avatar{width:42px;height:42px;display:grid;place-items:center;border:1px solid rgba(244,183,40,.28);border-radius:50%;background:#11100d;color:#d9b34f;font-size:.6rem;box-shadow:0 5px 22px rgba(0,0,0,.32)}.person.root .avatar{width:48px;height:48px;border-color:rgba(244,183,40,.5);background:#15130d}.person.mini .avatar{width:35px;height:35px}.person b{font-size:.49rem}.person small{color:#746f67;font-size:.4rem}.person.arriving{animation:memberArrive .9s ease-out}.emptySlot,.pendingSlot{min-width:68px;opacity:.72;color:#a79a79}.slotAvatar{width:32px;height:32px;display:grid;place-items:center;border:1px dashed rgba(244,183,40,.25);border-radius:50%;background:rgba(244,183,40,.018);animation:slotBreath 8s ease-in-out infinite}.slotAvatar.pending{animation:none;border-style:solid}.slotAvatar i{font-style:normal;color:#a88d4a;font-size:.78rem}.emptySlot b,.pendingSlot b{font-size:.43rem}.emptySlot small,.pendingSlot small{font-size:.36rem;color:#5e594f}.hud{position:absolute;z-index:8;top:10px;left:10px;display:flex;gap:7px;padding:5px 7px;border:1px solid rgba(255,255,255,.055);border-radius:9px;background:rgba(10,10,8,.82)}.hud span,.hud small{font-size:.43rem;color:#817a70}.rootPager{position:absolute;z-index:9;top:10px;right:10px;display:flex;align-items:center;gap:4px;padding:3px 5px;border:1px solid rgba(255,255,255,.055);border-radius:9px;background:rgba(10,10,8,.82)}.rootPager button{width:24px;height:24px;color:#a79f92}.rootPager span{font-size:.42rem;color:#777168}.backNetwork,.clearFocus{position:absolute;z-index:9;top:44px;height:29px;padding:0 8px;border:1px solid rgba(255,255,255,.06);border-radius:9px;background:rgba(12,12,9,.9);color:#a7a095;font-size:.48rem}.backNetwork{left:10px}.clearFocus{right:10px}.gestureHint{position:absolute;z-index:8;left:50%;top:82px;transform:translateX(-50%);display:flex;align-items:center;gap:8px;padding:7px 9px;border:1px solid rgba(244,183,40,.1);border-radius:11px;background:rgba(12,11,8,.88);pointer-events:none}.gestureHint b{color:#c59b3b}.gestureHint span{font-size:.45rem;color:#7a7469;line-height:1.45}.emptyLoad{position:absolute;z-index:14;left:50%;top:44%;transform:translate(-50%,-50%);display:grid;gap:9px;justify-items:center;color:#8d8579;font-size:.55rem}.emptyLoad button{height:30px;padding:0 9px;border:1px solid rgba(244,183,40,.16);border-radius:8px;color:#c7a451}.slotCard{position:absolute;z-index:20;left:50%;bottom:78px;transform:translateX(-50%);width:min(calc(100% - 28px),370px);padding:11px;border:1px solid rgba(255,255,255,.07);border-radius:15px;background:rgba(13,13,10,.96);box-shadow:0 14px 40px rgba(0,0,0,.38)}.slotCard>div:first-child{display:flex;align-items:center;gap:9px}.cardAvatar{width:30px;height:30px;display:grid;place-items:center;border:1px solid rgba(244,183,40,.24);border-radius:50%;color:#cba54a;font-size:.48rem}.slotCard strong{display:block;font-size:.58rem}.slotCard small{color:#746f67;font-size:.42rem}.slotCard>div:first-child>button{margin-left:auto;color:#756f65;font-size:.9rem}.slotCard p{margin:9px 0;color:#8c8579;font-size:.48rem;line-height:1.55}.cardActions{display:flex;gap:6px;flex-wrap:wrap}.cardActions button{flex:1 1 105px;height:32px;border-radius:9px;font-size:.46rem}.branchCta{border:1px solid rgba(244,183,40,.18);background:rgba(244,183,40,.07);color:#cfaa50}.moreCta{border:1px solid rgba(197,160,74,.12);background:rgba(197,160,74,.035);color:#aa9156}.networkCta{border:1px solid rgba(255,255,255,.06);background:rgba(255,255,255,.025);color:#979084}.inviteToast{position:absolute;z-index:22;left:50%;bottom:128px;transform:translateX(-50%);max-width:calc(100% - 30px);padding:7px 9px;border:1px solid rgba(244,183,40,.14);border-radius:9px;background:rgba(17,15,10,.95);color:#c9ad6d;font-size:.46rem;text-align:center}.controls{position:absolute;z-index:10;right:10px;bottom:max(14px,env(safe-area-inset-bottom));display:flex;gap:6px}.controls button{height:32px;border:1px solid rgba(255,255,255,.065);border-radius:10px;background:rgba(12,12,9,.9);color:#9a9388}.youButton{padding:0 9px;display:flex;align-items:center;gap:5px}.youButton span{color:#c5a04a}.youButton b{font-size:.44rem}.zoomControl{width:32px;font-size:.78rem}.tips{margin-top:8px;display:grid;grid-template-columns:repeat(3,1fr);gap:6px}.tips span{min-height:49px;padding:8px;border:1px solid rgba(255,255,255,.045);border-radius:11px;background:rgba(255,255,255,.015);color:#6f6a62;font-size:.43rem;line-height:1.45}.tips b{display:block;margin-bottom:2px;color:#9b9282;font-size:.45rem}@keyframes slotBreath{0%,72%,100%{box-shadow:0 0 0 rgba(244,183,40,0)}78%{box-shadow:0 0 15px rgba(244,183,40,.16)}84%{box-shadow:0 0 0 rgba(244,183,40,0)}}@keyframes memberArrive{0%{opacity:.25;transform:translate(-50%,-50%) scale(.82)}65%{opacity:1;transform:translate(-50%,-50%) scale(1.05)}100%{transform:translate(-50%,-50%) scale(1)}}@keyframes nodeReveal{0%{opacity:0;transform:translate(-50%,-50%) scale(.90)}100%{opacity:1;transform:translate(-50%,-50%) scale(1)}}@keyframes edgeReveal{0%{opacity:.12}100%{opacity:1}}@keyframes tailReveal{0%{opacity:0}100%{opacity:1}}@media(max-width:520px){.qaPage{padding-top:7px}.notice,.networkShell,.tips,.errorStrip,.staleStrip{width:calc(100vw - 12px)}.notice{padding:8px}.notice span{display:none}.stage{height:calc(100svh - 128px);min-height:560px}.tips{display:none}.controls{bottom:max(22px,calc(env(safe-area-inset-bottom) + 12px))}}@media(prefers-reduced-motion:reduce){.world.cameraMoving{transition:none}.slotAvatar,.person,.person.arriving,.treeEdge,.continuationTail{animation:none}.availableFlow{display:none}}
      `}</style>
    </main>
  );
}
