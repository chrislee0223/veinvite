...
import {
  NETWORK_CANVAS_COPY,
  VeInviteNetworkCanvas,
  type NetworkData,
  type NetworkMemberStatus,
  type NetworkSource,
} from '@/components/network/VeInviteNetworkCanvas';
...
function buildGraph(total: number) {
  ...
  for (let index = 2; index <= total + 1; index += 1) {
    ...
    const parentIndex = index <= 7 ? 1 : 2 + ((index * 7) % (index - 2));
    const parent = wallet(parentIndex);
    const status: NetworkMemberStatus = index % 5 === 0 ? 'IN_PROGRESS' : index % 3 === 0 ? 'QUALIFIED' : 'REWARDED';
    nodes.set(id, { wallet: id, parent, status, joinedAt: new Date(Date.UTC(2026, 7, 28) + index * 5400000).toISOString(), children: [] });
    nodes.get(parent)?.children.push(id);
  }
  return { root, nodes, nextIndex: total + 2 };
}

function descendants(nodes: Map<string, Node>, walletId: string) {
  const result: string[] = [];
  const queue = [...(nodes.get(walletId)?.children ?? [])];
  while (queue.length) {
    const id = queue.shift()!;
    if (result.includes(id)) continue;
    result.push(id);
    queue.push(...(nodes.get(id)?.children ?? []));
  }
  return result;
}
...
function readData(graph: ReturnType<typeof buildGraph>, focus: string, query = ''): NetworkData {
  ...
  const searchResults = value.length >= 3 ? rootBelow.filter((id) => id.includes(value)).slice(0, 8).map((id) => ({ wallet: id, parentWallet: nodes.get(id)?.parent ?? null, depth: lineage(nodes, id).length - 1 })) : [];
  return {
...
export function QaNetworkReleaseCandidateV51() {
  ...
  const source = useCallback<NetworkSource>(async (_root, options = {}) => {
    await new Promise<void>((resolve, reject) => {
      const timer = window.setTimeout(resolve, 70);
  ...
  const invite = useCallback(async (focus: string) => {
    ...
    const index = graph.nextIndex++;
    const id = wallet(index);
    const node: Node = { wallet: id, parent: focus, status: 'IN_PROGRESS', joinedAt: new Date().toISOString(), children: [] };
  ...
  return (
    <main className="candidatePage">
      <div className="qaStrip">
        <div><b>FINAL QA · Release candidate</b><span>Same interaction core intended for production integration · sample sponsor graph only</span></div>
        <div className="qaActions">
          {[30, 100, 500].map((value) => <button key={value} type="button" className={size === value ? 'active' : ''} onClick={() => { setSize(value as 30 | 100 | 500); setRevision((r) => r + 1); }}>{value}</button>)}
          <i />
