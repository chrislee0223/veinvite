...
type DragState = { key: string; offsetX: number; offsetY: number } | null;

const ROOT = 'root';
const STORAGE_KEY = 'veinvite:qa:radial-v29:positions';
...
const DESKTOP_ANCHORS: Point[] = [
  { x: -226, y: -58 }, { x: 226, y: -48 }, { x: -250, y: 62 }, { x: 244, y: 76 }, { x: -194, y: 162 },
  { x: 184, y: 174 }, { x: -112, y: 208 }, { x: 112, y: 202 }, { x: -270, y: 142 }, { x: 266, y: 146 },
];
const MOBILE_ANCHORS: Point[] = [
  { x: -106, y: -74 }, { x: 108, y: -66 }, { x: -118, y: 40 }, { x: 116, y: 50 },
  { x: -102, y: 144 }, { x: 98, y: 150 }, { x: -34, y: 186 }, { x: 42, y: 180 },
];
...
function basePersonPoint(id: string, localIndex: number, compact: boolean) {
  ...
  const jitterX = ((hash % 17) - 8) * (compact ? 0.75 : 1.45);
  const jitterY = (((hash >>> 6) % 17) - 8) * (compact ? 0.65 : 1.2);
...
export function QaNetworkRadialPlaygroundV29() {
  ...
  const layoutPrefix = `${scenario.id}|${center.id}|${compact ? 'm' : 'd'}|`;
  ...
  const personItems = useMemo<RingItem[]>(() => visibleChildren.map((id, localIndex) => {
    ...
    const custom = savedPositions[`${layoutPrefix}person|${id}`];
  ...
  const slotItems = useMemo<RingItem[]>(() => defaultGates.map((automatic, index) => {
    const id = `slot-${index}`;
    const custom = savedPositions[`${layoutPrefix}slot|${id}`];
    const point = custom ?? automatic;
    return { id, kind: 'slot', x: point.x, y: point.y, index };
  }), [defaultGates, savedPositions, layoutPrefix]);
  ...
  const zoomClass = zoom < .64 ? 'zoom-far' : zoom < .8 ? 'zoom-mid' : 'zoom-near';
  ...
  const goParent = () => {
    ...
    const target = basePersonPoint(center.id, localIndex, compact);
    clearTimers();
    setInfoOpen(false);
  ...
  const beginDrag = (event: ReactPointerEvent<HTMLButtonElement>, item: RingItem) => {
    if (!editMode || busy) return;
    event.preventDefault();
    event.stopPropagation();
    const pointer = logicalPointer(event.clientX, event.clientY);
    const key = layoutKey(item.kind, item.id);
    dragRef.current = { key, offsetX: pointer.x - item.x, offsetY: pointer.y - item.y };
    setDraggingKey(key);
  ...
  const dragNode = (event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    event.preventDefault();
    const pointer = logicalPointer(event.clientX, event.clientY);
    const xLimit = compact ? 138 : 306;
    const yMin = compact ? -210 : -238;
    const yMax = compact ? 220 : 246;
    const next = { x: clamp(pointer.x - dragging.offsetX, -xLimit, xLimit), y: clamp(pointer.y - dragging.offsetY, yMin, yMax) };
    setSavedPositions((current) => ({ ...current, [dragging.key]: next }));
  };
  ...
  return (
    <main className="page">
      <section className="labHeader">
        <div>
          <strong>RADIAL NETWORK PLAYGROUND · V29</strong>
          <span>Stable anchors · add-without-reflow · editable people + Available · live-following lines</span>
        </div>
      ...
      <section className="networkShell">
        <div className="networkTop">
          ...
          <div className="navActions">
            <button type="button" className={editMode ? 'active editButton' : 'editButton'} onClick={() => setEditMode((value) => !value)} disabled={busy}>{editMode ? '✓ Done' : '✦ Edit layout'}</button>
        ...
        <div ref={stageRef} className={`stage ${editMode ? 'editMode' : ''} phase-${phase} ${pageMotion ? `page-${pageMotion}` : ''}`} onWheel={onWheelZoom}>
          ...
          <div className={`scene ${zoomClass}`} style={{ '--sceneScale': zoom } as CSSProperties}>
            ...
            <svg className="edges" viewBox="-360 -285 720 620" aria-hidden="true">
              <defs>
                <linearGradient id="base29" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="rgba(231,204,126,.145)"/><stop offset="1" stopColor="rgba(231,204,126,.04)"/></linearGradient>
                <filter id="blur29" x="-100%" y="-100%" width="300%" height="300%"><feGaussianBlur stdDeviation="5"/></filter>
                {slotItems.map((slot, index) => <radialGradient key={`gradient-${slot.id}`} id={`gateGlow29-${index}`} gradientUnits="userSpaceOnUse" cx="0" cy="0" r={compact ? 50 : 70}><stop offset="0" stopColor="rgba(255,207,78,.84)"/><stop offset=".38" stopColor="rgba(244,183,40,.27)"/><stop offset="1" stopColor="rgba(244,183,40,0)"/><animate attributeName="cx" values={`0;${slot.x * .5};${slot.x}`} dur="8s" begin={`${index * 4}s`} repeatCount="indefinite"/><animate attributeName="cy" values={`0;${slot.y * .5};${slot.y}`} dur="8s" begin={`${index * 4}s`} repeatCount="indefinite"/></radialGradient>)}
              </defs>
              {ringItems.map((item) => {
                const key = layoutKey(item.kind, item.id);
                return <path key={`${item.id}-base`} d={pathFor(item, lineStyle)} className={`spoke ${item.kind === 'slot' ? 'slotSpoke' : ''} ${draggingKey ? (draggingKey === key ? 'dragActive' : 'dragDim') : ''}`}/>;
              })}
              {slotItems.map((slot, index) => <path key={`${slot.id}-glow`} d={pathFor(slot, lineStyle)} className={`gateGlowPath ${draggingKey && draggingKey !== layoutKey('slot', slot.id) ? 'dragDim' : ''}`} stroke={`url(#gateGlow29-${index})`} filter="url(#blur29)"><animate attributeName="opacity" values="0;0;.82;.48;0;0" keyTimes="0;.08;.18;.42;.56;1" dur="8s" begin={`${index * 4}s`} repeatCount="indefinite"/></path>)}
            </svg>
            ...
            <div className="ringLayer">
              {personItems.map((item, index) => {
                ...
                return <button key={item.id} type="button" className={`ringNode person ${newArrival === item.id ? 'newArrival' : ''} ${draggingKey === key ? 'dragging' : ''}`} style={style} onClick={() => enterNode(item)} onPointerDown={(event) => beginDrag(event, item)} onPointerMove={dragNode} onPointerUp={endDrag} onPointerCancel={endDrag} disabled={busy}><span className="floatInner"><span className="ringCircle">●</span><b>{shortId(item.id)}</b><small>{child?.children.length ?? 0} direct · {childNetwork} net</small></span></button>;
              })}
              {slotItems.map((item, index) => {
                ...
                return <button key={item.id} type="button" className={`ringNode slot ${active ?? ''} ${draggingKey === key ? 'dragging' : ''}`} style={style} onClick={() => simulateGate(item.id)} onPointerDown={(event) => beginDrag(event, item)} onPointerMove={dragNode} onPointerUp={endDrag} onPointerCancel={endDrag} disabled={busy && !active}><span className="floatInner"><span className="slotCircle">{active === 'joining' ? '…' : active === 'complete' ? '✓' : '+'}</span><b>{active === 'joining' ? 'Joining' : active === 'complete' ? 'Completed' : 'Available'}</b><small>{editMode ? 'Drag anywhere' : active ? 'Gate demo' : 'Invite gate'}</small></span></button>;
              })}
            </div>
          ...
          {editMode ? <div className="editHint">Drag people or Available · overlaps are allowed · lines follow automatically</div> : <div className="zoomHint">wheel / − + · {Math.round(zoom * 100)}%</div>}
          {arrivalNotice ? <div className="arrivalNotice">✓ {arrivalNotice}</div> : null}
          {debug ? <div className="debugPanel"><span>People/page {peoplePerPage}</span><span>Available {slotCount}</span><span>Direct {childCount}</span><span>Pages {pageCount}</span><span>Zoom {Math.round(zoom * 100)}%</span><span>{editMode ? 'EDIT' : 'VIEW'}</span><span>Saved {Object.keys(savedPositions).length}</span></div> : null}
        </div>
      </section>

      <section className="rules">
        <span><b>Stable anchors</b>새 사람이 추가돼도 먼저 있던 사람의 기본 위치는 다시 계산하지 않음</span>
        <span><b>Add without disappearing</b>현재 페이지에 자리가 있으면 새 사람만 합류 · 꽉 차면 기존 화면 유지</span>
        <span><b>Free edit</b>편집 모드에서는 사람과 Available 모두 자유 이동 · 겹침도 허용</span>
        <span><b>Lines follow</b>노드 이동 중 연결선과 Available 빛도 실시간으로 새 위치를 따라감</span>
      </section>
      ...
      <style jsx>{`
        ...
        .slot{opacity:.82}.slotCircle{width:46px;height:46px;border:1px dashed rgba(226,181,62,.5);color:#c79f36;font-size:.9rem;animation:gatePulse29 8s ease-in-out infinite;animation-delay:var(--gateDelay,0s)}.slot b{color:#a3936e}.slot small{color:#675f53}.slot.joining .slotCircle{border-style:solid;animation:joining29 .9s ease-in-out infinite}.slot.complete .slotCircle{border-style:solid;border-color:rgba(138,205,123,.68);color:#8fca80;box-shadow:0 0 22px rgba(115,189,101,.1)}.newArrival .ringCircle{animation:newArrival29 650ms cubic-bezier(.16,.8,.2,1) both}
        ...
        .traveler{--tx:0px;--ty:0px;position:absolute;left:50%;top:50%;width:110px;display:grid;justify-items:center;gap:4px;z-index:30;color:#e8dfcb;pointer-events:none}.travelCircle{width:56px;height:56px;border:1px solid rgba(244,183,40,.72);color:#e5b943;box-shadow:0 0 34px rgba(244,183,40,.09)}.traveler b{font-size:.49rem}.traveler.in{animation:travelIn29 500ms cubic-bezier(.18,.78,.18,1) both}.traveler.out{animation:travelOut29 500ms cubic-bezier(.18,.78,.18,1) both}
        .phase-depart .ringLayer,.phase-depart .edges{opacity:.12}.phase-depart .centerWrap{animation:centerYield29 500ms cubic-bezier(.18,.78,.18,1) both}.phase-arrive .centerWrap{animation:centerArrive29 330ms cubic-bezier(.18,.78,.18,1) both}.phase-arrive .ringNode{animation:bloom29 430ms cubic-bezier(.18,.8,.24,1) both;animation-delay:var(--delay,0ms)}.page-left .ringLayer,.page-right .ringLayer{animation:pageFade29 300ms ease both}
        ...
        .scene.zoom-mid .ringNode small,.scene.zoom-mid .centerFloat small,.scene.zoom-mid .parentNode small{display:none}.scene.zoom-far .ringNode small,.scene.zoom-far .centerFloat small,.scene.zoom-far .parentNode small,.scene.zoom-far .emptyHint small{display:none}.scene.zoom-far .spoke{opacity:.72}.scene.zoom-far .centerCircle{box-shadow:none}.scene.zoom-far .waterGlow{opacity:.55}
        ...
        .zoomHint,.editHint{position:absolute;right:10px;bottom:10px;padding:5px 7px;border:1px solid rgba(255,255,255,.04);border-radius:8px;background:rgba(12,12,10,.66);font-size:.38rem;color:#5f594f;z-index:42;pointer-events:none}.editHint{color:#a08b58;border-color:rgba(244,183,40,.11)}.arrivalNotice{position:absolute;left:50%;bottom:38px;transform:translateX(-50%);padding:7px 10px;border:1px solid rgba(138,205,123,.2);border-radius:10px;background:rgba(10,16,10,.9);color:#91c886;font-size:.43rem;z-index:44;animation:notice29 2200ms ease both}.debugPanel{position:absolute;left:10px;bottom:10px;display:flex;flex-wrap:wrap;gap:5px;max-width:70%;z-index:45}.debugPanel span{padding:5px 7px;border:1px solid rgba(255,255,255,.05);border-radius:8px;background:rgba(12,12,10,.84);font-size:.4rem;color:#777067}
        ...
        @keyframes float29{0%,100%{transform:translate3d(0,-1px,0) rotate(-.22deg)}35%{transform:translate3d(1.2px,2.4px,0) rotate(.26deg)}70%{transform:translate3d(-1px,.5px,0) rotate(-.1deg)}}
        @keyframes centerFloat29{0%,100%{transform:translateY(0)}45%{transform:translateY(1.7px)}75%{transform:translateY(-1px)}}
        @keyframes bloom29{from{opacity:0;filter:blur(2px);transform:translate(calc(var(--x) - 50%),calc(var(--y) - 46%)) scale(.94)}to{opacity:1;filter:blur(0);transform:translate(calc(var(--x) - 50%),calc(var(--y) - 50%)) scale(1)}}
        @keyframes gatePulse29{0%,68%,100%{box-shadow:0 0 0 rgba(244,183,40,0);border-color:rgba(226,181,62,.42)}78%{box-shadow:0 0 22px rgba(244,183,40,.1);border-color:rgba(238,191,64,.68)}}
        @keyframes joining29{0%,100%{transform:scale(1);box-shadow:0 0 10px rgba(244,183,40,.03)}50%{transform:scale(1.06);box-shadow:0 0 24px rgba(244,183,40,.13)}}
        @keyframes newArrival29{0%{opacity:.2;transform:translateY(10px) scale(.72);box-shadow:0 0 0 rgba(244,183,40,0)}60%{opacity:1;transform:translateY(-2px) scale(1.08);box-shadow:0 0 34px rgba(244,183,40,.15)}100%{opacity:1;transform:translateY(0) scale(1);box-shadow:0 0 12px rgba(244,183,40,.04)}}
        @keyframes travelIn29{0%{transform:translate(calc(var(--tx) - 50%),calc(var(--ty) - 50%)) scale(1)}38%{transform:translate(calc(var(--tx)*.62 - 50%),calc(var(--ty)*.62 - 58%)) scale(1.08)}100%{transform:translate(-50%,-50%) scale(1.05)}}
        @keyframes travelOut29{0%{transform:translate(-50%,-50%) scale(1.05)}62%{transform:translate(calc(var(--tx)*.65 - 50%),calc(var(--ty)*.65 - 56%)) scale(1.04)}100%{transform:translate(calc(var(--tx) - 50%),calc(var(--ty) - 50%)) scale(1)}}
        @keyframes centerYield29{to{opacity:.12;transform:translate(-50%,-82%) scale(.72)}}@keyframes centerArrive29{from{opacity:.2;transform:translate(-50%,-43%) scale(.92)}to{opacity:1;transform:translate(-50%,-50%) scale(1)}}@keyframes pageFade29{0%{opacity:.22;filter:blur(1px)}100%{opacity:1;filter:blur(0)}}@keyframes notice29{0%{opacity:0;transform:translate(-50%,7px)}12%,78%{opacity:1;transform:translate(-50%,0)}100%{opacity:0;transform:translate(-50%,-4px)}}
        @media(max-width:640px){.page{padding-top:8px}.labHeader,.scenarioBar,.controlBar,.networkShell,.rules{width:min(calc(100vw - 12px),920px)}.labHeader{padding:8px 9px}.labHeader span{display:none}.networkTop{align-items:flex-start;flex-direction:column;padding:8px 9px}.navActions{width:100%;overflow-x:auto;scrollbar-width:none}.navActions::-webkit-scrollbar{display:none}.stage{height:min(70svh,610px);min-height:510px}.edges{width:520px;height:520px}.centerCircle{width:66px;height:66px}.centerFloat{width:116px}.ringNode{width:94px}.ringCircle{width:47px;height:47px}.slotCircle{width:43px;height:43px}.parentNode{top:calc(50% - 194px)}.rules{grid-template-columns:1fr 1fr}.infoCard{left:8px;right:8px;top:auto;bottom:8px;width:auto}.editHint{left:10px;right:auto;max-width:72%}}
        @media(prefers-reduced-motion:reduce){.centerFloat,.floatInner,.slotCircle,.traveler,.newArrival .ringCircle,.phase-depart .centerWrap,.phase-arrive .centerWrap,.page-left .ringLayer,.page-right .ringLayer,.arrivalNotice{animation:none!important}.scene{transition:none}.gateGlowPath{display:none}}
      `}</style>
    </main>
  );
}
