...
- The Production public-reporting baseline is intentionally still disabled. The one-way round baseline lock and immutable report system are deployed, but the operator must explicitly choose the first official VeBetterDAO round before public cumulative numbers can be published.
