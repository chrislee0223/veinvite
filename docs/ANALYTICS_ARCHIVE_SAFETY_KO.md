...
- `HOT_SOURCE_PURGED`는 실제 bounded compaction 함수만 생성
- compaction 함수는 service-only이며 postgres owner 권한으로 실행
...
다음이 준비되기 전까지 destructive cleanup을 자동화하지 않습니다.
...
실제 object를 저장하고 다시 읽어 checksum/건수/스키마를 검증한 뒤에만 destructive cleanup을 활성화합니다.
