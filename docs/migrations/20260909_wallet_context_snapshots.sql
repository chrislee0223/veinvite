-- VeInvite wallet context tracking hardening.
--
-- Goals:
-- 1. Track authenticated wallet language on every route, including /i/* and /r/*.
-- 2. Preserve explicit/manual language over lower-confidence automatic sources.
-- 3. Support query-param language provenance without conflating it with nationality.
-- 4. Freeze accepted-referral language to activation-time facts whenever trustworthy
--    activation evidence exists; never invent historical values for missing facts.
-- 5. Keep country reporting based on trusted country evidence only. Language must
--    never be used as a country proxy.
