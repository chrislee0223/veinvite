-- Create the immutable previous-round paid-referral baseline on demand once the
-- current round has at least one finalized paid result. This removes the gap
-- between VeBetterDAO round turnover and the daily reconciliation cron without
-- changing reward settlement or allowing public callers to write snapshots.
...
begin
  ...
  -- Waiting for a finalized settlement after the previous round boundary means
  -- the payout worker has advanced beyond any older pre-boundary transaction
  -- before this historical paid-referral ranking is frozen.
  if not exists (
    select 1
