...
test('the mission policy patch is loaded after shared and UI mission copy polish', () => {
  const hardeningIndex = providersSource.indexOf(
    "import '@/lib/i18n/copyHardening';",
  );
  const missionCopyIndex = providersSource.indexOf(
  ...
  assert.ok(policyIndex > missionCopyIndex);
});
