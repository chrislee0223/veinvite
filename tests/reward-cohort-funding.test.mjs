...
const cohortMigration = await readFile(
  new URL('../supabase/migrations/20260904203001_bind_reward_cohorts_and_promo_funding.sql', import.meta.url),
  'utf8',
);
...
const provenanceMigration = await readFile(
  new URL('../supabase/migrations/20260904203300_harden_reward_cohort_binding_provenance.sql', import.meta.url),
  'utf8',
);
const claimedCohortMigration = await readFile(
  new URL('../supabase/migrations/20260904203400_harden_claimed_cohort_selection.sql', import.meta.url),
  'utf8',
);
...
test('invitation cohort and source allocation are permanently paired', () => {
  ...
  assert.match(roundSafeMigration, /REWARD_COHORT_BINDING_IMMUTABLE/);
  assert.match(roundSafeMigration, /v_receipt\.vebetter_round_id \+ 1 <> new\.reward_cohort_round_id/);
});
...
test('bootstrap carry-forward is explicit without rewriting original entry evidence', () => {
  assert.match(provenanceMigration, /reward_cohort_binding_source/);
  assert.match(provenanceMigration, /BOOTSTRAP_CARRY_FORWARD/);
  assert.match(provenanceMigration, /e\.details ->> 'currentRoundId' = '113'/);
  assert.match(provenanceMigration, /ROUND_MATCH/);
  assert.match(provenanceMigration, /aa_invitations_bind_reward_cohort/);
  assert.match(provenanceMigration, /execute function public\.bind_invitation_reward_cohort/);
});
...
test('fixed cohort pricing excludes unbound pending invites from newer rounds', () => {
  assert.match(predictivePlanning, /includePendingAcceptance\?: boolean/);
  assert.match(predictivePlanning, /pendingAcceptanceCount: 0/);
  assert.match(reservationServer, /includePendingAcceptance: false/);
});

test('legacy automatic payout worker is safely routed to a valid claimed cohort', () => {
  ...
  assert.match(claimedCohortMigration, /q\.claim_requested_by_wallet = q\.recipient_wallet/);
  assert.match(claimedCohortMigration, /i\.status = 'COMPLETED'/);
  assert.match(claimedCohortMigration, /i\.sybil_status = 'CLEAR'/);
});
