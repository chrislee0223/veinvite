...
test('slot provenance is append-only in the unified event ledger', () => {
  assert.match(
    auditMigration,
    /INVITE_SLOT_RESERVED/i,
  );
  assert.match(
    auditMigration,
    /INVITE_SLOT_BACKFILLED/i,
  );
  assert.match(
    auditMigration,
    /'invite_slot', i\.invite_slot/i,
  );
  assert.match(
    auditMigration,
    /'slot_semantics', 'invitation_concurrency'/i,
  );
  assert.match(
    auditMigration,
    /REFERRAL_PLACEMENT_SLOT_ASSIGNED/i,
  );
  assert.match(
    auditMigration,
    /'slot_semantics', 'network_placement'/i,
  );
  assert.doesNotMatch(
    auditMigration,
