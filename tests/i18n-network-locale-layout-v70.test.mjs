...
const [source, guideSource, v69Source, controlCopySource] = await Promise.all([
...
test('V70 is the final Network canary layer and preserves the mature V69 chain', () => {
  assert.match(guideSource, /AppNetworkCanaryV70/);
  assert.match(guideSource, /<AppNetworkCanaryV70 key=\{wallet\.toLowerCase\(\)\} locale=\{locale\} \/>/);
